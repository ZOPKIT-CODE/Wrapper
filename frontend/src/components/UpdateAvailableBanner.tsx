...
export function UpdateAvailableBanner() {
  ...
  // Browser timer id (window.setInterval returns a number in the DOM lib).
  ...
  const handleUpdateAvailable = useCallback((isForced: boolean) => {
    ...
    const attemptedExpiry = Number(
      localStorage.getItem(ATTEMPTED_VERSION_EXPIRY_KEY) || '0'
    )
    const polled = readPolledVersion()
    if (
      attempted &&
  ...
  return (
    <div
      ...
      className={[
        ...
        'sm:right-auto sm:left-1/2 sm:max-w-md sm:-translate-x-1/2',
