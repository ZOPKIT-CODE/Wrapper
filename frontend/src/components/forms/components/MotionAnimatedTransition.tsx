 ...
 */
export const MotionAnimatedTransition: React.FC<
  MotionAnimatedTransitionProps
> = ({ children, direction = 'none', className }) => {
  ...
  return (
    <div ref={skeletonRef} className={cn('space-y-2', className)}>
      {Array.from({ length: count }, (_, index) => (
        <div
          key={index}
          className="skeleton-item h-4 animate-pulse rounded bg-gray-200"
  ...
  return (
    ...
    >
      {Array.from({ length: totalSteps }, (_, index) => (
        <div
          key={index}
          className="step-item flex h-8 w-8 items-center justify-center rounded-full border-2 text-sm font-medium transition-all duration-300"
        >
...
export const MotionSuccessMessage: React.FC<{
  ...
  return (
    <div
      ref={messageRef}
      className={cn('mt-1 flex items-center text-sm text-green-600', className)}
    >
