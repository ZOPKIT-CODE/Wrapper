...
export class CrossFieldValidation {
  ...
  static createCrossFieldSchema(
    fieldValidations: Record<
      string,
      (values: FormValues) => z.ZodSchema<unknown>
    >
  ) {
    ...
    return baseSchema.refine(
      (data) => {
        ...
        return Object.entries(fieldValidations).every(
          ([fieldId, validator]) => {
            try {
  ...
  static numberRange(minField: string, maxField: string) {
    return z
      ...
      .refine((data) => data[minField] <= data[maxField], {
      ...
      .refine(
        (data) => {
          const sum = fields.reduce(
            (total, field) =>
              total + ((data as Record<string, number>)[field] || 0),
            0
          )
          return sum === target
        },
...
export class AsyncValidation {
  ...
  static async usernameAvailable(username: string): Promise<boolean> {
    try {
      const response = await fetch(
        `${import.meta.env.VITE_API_BASE_URL ?? ''}/api/check-username?username=${encodeURIComponent(username)}`
      )
      return response.ok
    } catch {
      return false
