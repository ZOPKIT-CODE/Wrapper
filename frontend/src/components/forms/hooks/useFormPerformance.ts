  ...
  const loadComponent = useCallback(
    async (fieldType: string) => {
      ...
      try {
        ...
        console.error(
          `Failed to load component for field type: ${fieldType}`,
          error
        )
      }
    },
...
export const useFormAnalytics = () => {
  ...
  const trackFieldInteraction = useCallback((fieldId: string) => {
    fieldInteractionsRef.current[fieldId] =
      (fieldInteractionsRef.current[fieldId] || 0) + 1
  }, [])

  // Get analytics data
  const getAnalytics = useCallback(() => {
    const totalTime = Date.now() - startTimeRef.current
    const stepTimes = Object.entries(stepTimesRef.current).map(
      ([step, time]) => ({
        step: parseInt(step),
        timeMs: time,
      })
    ...
    const fieldInteractions = Object.entries(fieldInteractionsRef.current).map(
      ([field, count]) => ({
        fieldId: field,
