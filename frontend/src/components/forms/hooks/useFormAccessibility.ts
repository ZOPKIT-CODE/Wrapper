...
export const useFormAccessibility = () => {
  ...
  const focusFirstField = useCallback(() => {
    const firstField = document.querySelector(
      '[data-form-container] input, [data-form-container] select, [data-form-container] textarea'
    ) as HTMLElement
    if (firstField) {
      firstField.focus()
    }
  ...
  const focusNextField = useCallback(() => {
    ...
    if (currentField) {
      const nextField = currentField.parentElement?.querySelector(
...
export const useFieldAccessibility = (fieldId: string, _fieldLabel: string) => {
  ...
  const getAriaAttributes = () => ({
    ...
    'aria-describedby':
      [hasError ? errorId : null, helpId].filter(Boolean).join(' ') ||
      undefined,
    'aria-required': true,
  })
