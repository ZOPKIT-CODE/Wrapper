...
export function PermissionGuard({
  ...
  const { isLoading, isAuthenticated, getPermission, getPermissions } =
    useAuth()
    ...
    async function checkPermission() {
      ...
      try {
        ...
        if (allPermissions?.permissions) {
          ...
          if (
            Array.isArray(allPermissions.permissions) &&
            ...
            if (typeof allPermissions.permissions[0] === 'string') {
              ...
              hasRequiredPermission =
                allPermissions.permissions.includes(requiredPermission)
            } else {
        ...
        if (!hasRequiredPermission) {
          ...
          logger.debug(
            '1. Verify permission exists in the IdP dashboard: Settings > Permissions'
          )
          logger.debug(
            '2. Check permission key matches exactly:',
            requiredPermission
          )
          logger.debug(
            '3. Ensure permission is assigned to user via role or directly'
          )
          logger.debug(
            '4. **IMPORTANT**: Log out and log back in to refresh permissions'
          )
