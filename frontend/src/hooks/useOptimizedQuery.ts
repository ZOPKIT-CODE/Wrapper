...
export function useOptimizedQuery<T>(options: UseOptimizedQueryOptions<T>) {
  ...
  const prefetch = useCallback(
    (prefetchQueryKey: string[], prefetchQueryFn: () => Promise<any>) => {
  ...
  return {
    ...
    cacheAge: cache.getTimestamp(cacheKey)
      ? Date.now() - cache.getTimestamp(cacheKey)!
      : null,
  }
}
...
export function useBatchedQueries<T>(
  ...
  const results = queries.map((query) =>
    // KNOWN VIOLATION (pre-existing): a hook in .map() is only safe while `queries`
    // is stable in length/order. Correct fix is TanStack `useQueries`, but that
    // changes the per-result API (refetch/invalidate/prefetch) that useDashboardData
    // relies on — deferred to avoid a risky refactor. Tracked as tech debt.
