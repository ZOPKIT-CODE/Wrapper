        ...
        <>
          <div style={{ borderRadius: 8, border: '1px solid var(--zk-line)', overflow: 'hidden' }}>
            <Table>
              <TableHeader>
                <TableRow style={{ background: 'var(--zk-bg-2)' }}>
                  <TableHead className="w-[240px] min-w-[180px]" style={TH_STYLE}>User</TableHead>
                  <TableHead className="hidden md:table-cell" style={TH_STYLE}>First name</TableHead>
                  <TableHead className="hidden md:table-cell" style={TH_STYLE}>Last name</TableHead>
                  <TableHead style={TH_STYLE}>Role</TableHead>
                  <TableHead style={TH_STYLE}>Status</TableHead>
                  <TableHead className="hidden md:table-cell" style={TH_STYLE}>Department</TableHead>
                  <TableHead className="hidden lg:table-cell" style={TH_STYLE}>Last Active</TableHead>
              ...
              <TableBody>
                {users.map((user) => {
                  ...
                  const removeId = user.userId ?? (user.invitationId ? `inv_${user.invitationId}` : null)
                  return (
                    <TableRow key={user.userId ?? user.invitationId} className="group">
                      <TableCell>
                      ...
                      <TableCell>
                        {user.isTenantAdmin ? (
                          <Badge variant="outline" className="border-border bg-muted font-medium text-[11px] text-primary">Admin</Badge>
                        ) : role ? (
                      ...
                      <TableCell>
                        <DropdownMenu modal={false}>
                          ...
                          <DropdownMenuContent align="end" className="z-50">
                            <DropdownMenuItem onClick={() => user.userId && onViewUser(user.userId)}>
                              <Eye className="mr-2 h-4 w-4" /> View Details
                            </DropdownMenuItem>
                            <DropdownMenuItem onClick={() => user.userId && onAssignRole(user.userId)}>
                              <Edit className="mr-2 h-4 w-4" /> Assign Role
                            </DropdownMenuItem>
                            <DropdownMenuSeparator />
                            <DropdownMenuItem
                              disabled={!user.userId}
                              onClick={() => user.userId && onToggleStatus(user.userId, user.isActive)}
                            >
                              {user.isActive !== false ? (
