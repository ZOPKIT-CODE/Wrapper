...
import {
  applyIndiaRegionalDefaultsIfMissing,
  resolveCountryCode,
...
export const useFormPersistence = ({
  ...
  const normalizeRestoredData = useCallback((rawData: any) => {
    ...
    normalized.businessDetails = {
      ...
      companyName:
        existingBusinessDetails.companyName ??
        normalized.companyName ??
        normalized.businessName,
      businessType:
        existingBusinessDetails.businessType ??
        normalized.businessType ??
        normalized.industry,
      organizationSize:
        existingBusinessDetails.organizationSize ??
        normalized.organizationSize ??
    ...
    normalized.defaultCurrency =
      normalized.defaultCurrency ?? normalized.currency
    normalized.defaultTimeZone =
      normalized.defaultTimeZone ?? normalized.timezone
    normalized.defaultLanguage =
      normalized.defaultLanguage ?? normalized.language
    ...
    normalized.billingAddress =
      normalized.billingAddress ?? normalized.billingStreet
    normalized.billingStreet =
      normalized.billingStreet ?? normalized.billingAddress
    normalized.state =
      normalized.state ??
      normalized.billingState ??
      normalized.incorporationState
      ...
      if (user?.email) {
        try {
          ...
          if (response?.data?.success) {
            onboardingLogger.debug('Form data saved to backend', {
              step: currentStep,
            })
          }
        } catch (error: unknown) {
          onboardingLogger.warn('Backend form data save failed', {
  ...
  const restoreFormData = useCallback(async () => {
    ...
    try {
      ...
      if (user?.email) {
        try {
          const backendResponse = await onboardingAPIOptimized.getDataByEmail(
            user.email,
            user.id
          )
          ...
          if (
            ...
            backendResponse?.data?.data?.savedFormData
          ) {
            restoredData = backendResponse.data.data.savedFormData
            const backendStep = backendResponse.data.data.onboardingStep
            if (backendStep) {
              restoredStep =
                parseInt(String(backendStep).replace('step_', '')) || 1
            }
            onboardingLogger.info('Restored form data from backend', {
              restoredStep,
            })
            ...
            restoredStep = onboardingData.currentStep
              ? parseInt(
                  String(onboardingData.currentStep).replace('step_', '')
                ) || 1
              : 1
            if (onboardingData.formData) {
              ...
              restoredData = {}
              Object.values(onboardingData.stepData).forEach(
                (stepData: unknown) => {
                  Object.assign(
                    restoredData!,
            ...
            onboardingLogger.info(
              'Restored form data from backend (legacy format)',
              { restoredStep }
            )
          }
        } catch (error) {
          onboardingLogger.warn(
            'Failed to restore from backend — falling back to localStorage draft',
            { error: String(error) }
          )
        }
      }
      ...
      if (!restoredData && user?.id) {
        ...
        if (draft) {
          ...
          onboardingLogger.info('Restored form data from localStorage draft', {
            restoredStep,
          })
        }
      }
      ...
      restoredData = normalizeRestoredData(restoredData) as Record<
        string,
        unknown
      >
        ...
        try {
          if (
            ...
            Object.keys(val as Record<string, unknown>).forEach((nestedKey) => {
              setFormValue(
                nestedKey,
                (val as Record<string, unknown>)[nestedKey],
                fullKey
              )
            })
          } else {
            form.setValue(
              fullKey as Parameters<typeof form.setValue>[0],
              val === undefined ? '' : val,
              {
  ...
  const clearFormData = useCallback(() => {
    try {
      if (user?.email) {
        onboardingAPIOptimized
          .updateStep('step_1', {}, user.email, {}, user.id)
          .catch(() => {})
      }
      if (user?.id) {
      ...
      restoreFormData().then((restoredStep) => {
        if (restoredStep !== currentStep && restoredStep > 1) {
          window.dispatchEvent(
            new CustomEvent('onboarding-step-restored', {
