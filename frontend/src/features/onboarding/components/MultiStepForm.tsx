...
export const MultiStepForm: React.FC<MultiStepFormProps> = ({
  ...
  const {
    ...
    userClassification,
    externalCurrentStep
  ) // FIXED: Pass external step as initial
  ...
  const handleValidationError = useCallback(
    (errors: FieldErrors<FieldValues>, stepNumber: number) => {
      const { message, fields } = formatValidationErrors(errors)
      onboardingLogger.warn('Validation error on step', {
        stepNumber,
        message,
        fieldPaths: fields.map((f: FieldMapping) => f.fieldPath),
  ...
  const handleNext = useCallback(async () => {
    onboardingLogger.debug('Next clicked', {
      currentStep,
      totalSteps: stepsConfig.length,
    })
    const success = await nextStep(handleValidationError)
    if (success) {
      onboardingLogger.info('Next step success', {
        from: currentStep,
        to: currentStep + 1,
      })
  ...
  const handlePrev = useCallback(() => {
    onboardingLogger.info('Prev clicked', {
      from: currentStep,
      to: currentStep - 1,
    })
    prevStep()
