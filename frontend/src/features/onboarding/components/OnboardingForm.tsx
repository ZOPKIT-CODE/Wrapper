...
export const verifyEmailDomain = (
  email: string
): { isDomainEmail: boolean; domain: string | null } => {
    ...
    if (explicitClassification) {
      ...
      if (
        validClassifications.includes(
          explicitClassification as UserClassification
        )
      ) {
...
export const OnboardingForm = () => {
  ...
  const [selectedFlow] = useState<'newBusiness' | 'existingBusiness'>(
    'newBusiness'
  )
  ...
  const { clearFormData, restoreFormData } = useFormPersistence({
    form,
  ...
  useEffect(() => {
    onboardingLogger.info('OnboardingForm mounted', {
      currentStep,
      selectedFlow,
    })
    ...
    restoreFormData()
      .then((restoredStep) => {
        ...
        if (restoredStep && restoredStep > 1 && restoredStep !== currentStep) {
          onboardingLogger.info('Form progress restored', {
            restoredStep,
          ...
          onboardingLogger.debug('No restore needed or same step', {
            restoredStep,
            currentStep,
          })
        }
      ...
      .catch((error) => {
        ...
        onboardingLogger.warn('Failed to restore form data', {
          error: String(error),
  ...
  useEffect(() => {
    ...
    window.addEventListener(
      'onboarding-step-restored',
      handleStepRestored as EventListener
    )
    return () => {
  ...
  const handleSubmit = useCallback(
    async (data: newBusinessData | existingBusinessData) => {
      onboardingLogger.info('Submit started', {
        currentStep,
        hasTermsAccepted: !!data.termsAccepted,
      })
      ...
      if (isRateLimited) {
        onboardingLogger.warn('Submit blocked: rate limited', {
      ...
      try {
        ...
        if (!rawCompanyName || rawCompanyName.trim() === '') {
          ...
          sonnerToast.error('Company Name Required', {
            description:
              'Please enter your company name in the Business Details step',
            duration: 6000,
          })
          return
        }
        ...
        if (!securityValidation.isValid) {
          onboardingLogger.warn('Submit blocked: security validation failed', {
        ...
        if (!companyName || companyName.trim() === '') {
          onboardingLogger.warn(
            'Submit blocked: company name empty after sanitization',
            {
              sanitizedData: {
                ...sanitizedData,
                businessDetails: sanitizedData.businessDetails,
              },
            }
          )
          setIsSubmitting(false)
          ...
          sonnerToast.error('Company Name Required', {
            description:
              'Company name was lost during sanitization. Please re-enter your company name.',
            duration: 6000,
          })
          return
        }
        ...
        const response = await api.post('/onboarding/onboard-frontend', {
          ...
          companySize:
            businessDetails.organizationSize || sanitizedData.organizationSize,
          businessType:
            businessDetails.businessType || sanitizedData.businessType,
          primaryUseCase:
            businessDetails.primaryUseCase || sanitizedData.primaryUseCase,
          website: filterEmpty(sanitizedData.website),

          // Admin/Contact Information
          firstName:
            sanitizedData.firstName || sanitizedData.personalDetails?.firstName,
          lastName:
            sanitizedData.lastName || sanitizedData.personalDetails?.lastName,
          email:
            sanitizedData.email ||
            sanitizedData.personalDetails?.email ||
            sanitizedData.adminEmail,
          adminEmail: sanitizedData.adminEmail,
          adminMobile: filterEmpty(
            sanitizedData.adminMobile ||
              sanitizedData.adminPhone ||
              sanitizedData.phone
          ),
          ...
          preferredContactMethod: filterEmpty(
            sanitizedData.preferredContactMethod
          ),
          ...
          hasGstin:
            Boolean(sanitizedData.gstin && sanitizedData.gstin.trim() !== '') ||
            Boolean(sanitizedData.vatGstRegistered),
        ...
        if (response.data.success) {
          onboardingLogger.info('Onboarding submit success', {
          ...
          await queryClient.fetchQuery({
            ...
            queryFn: async () => {
              const { default: apiFetcher } = await import('@/lib/api/client')
          ...
          const baseUrl =
            response.data?.data?.redirectUrl || '/dashboard/applications'
          const url =
            baseUrl.startsWith('/dashboard') &&
            !baseUrl.includes('onboarding=complete')
              ? baseUrl.includes('?')
          ...
          onboardingLogger.error('Onboarding API returned success=false', {
            message: msg,
            responseData: response.data,
          })
          throw new Error(msg)
