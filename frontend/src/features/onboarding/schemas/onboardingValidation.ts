...
export const onboardingFormSchema = z
  .object({
    ...
    cinNumber: z
      .string()
      .max(50, 'CIN number must be less than 50 characters')
      .optional(),
    ...
    billingCity: z
      ...
      .refine(
        (val) => val.trim().length > 0,
        'Billing city cannot be only whitespace'
      ),
    billingZip: z
      .string()
      ...
      .refine(
        (val) => val.trim().length > 0,
        'Billing zip code cannot be only whitespace'
      ),
  ...
  .superRefine((data, ctx) => {
      ...
      if (country === 'IN') {
        if (!data.gstin || data.gstin.trim() === '') {
          ...
          if (!gstinResult.success) {
            ctx.addIssue({
              code: z.ZodIssueCode.custom,
              message:
                gstinResult.error.errors[0]?.message || 'Invalid GSTIN format',
              path: ['gstin'],
            })
          }
        }
        ...
        if (!data.vatNumber || data.vatNumber.trim() === '') {
          ctx.addIssue({
            code: z.ZodIssueCode.custom,
            message:
              'VAT number is required when VAT/GST Registered is enabled',
            path: ['vatNumber'],
          })
        }
      }
        ...
        if (!data.panNumber || data.panNumber.trim() === '') {
          ...
          if (!panResult.success) {
            ctx.addIssue({
              code: z.ZodIssueCode.custom,
              message:
                panResult.error.errors[0]?.message || 'Invalid PAN format',
              path: ['panNumber'],
            })
          }
        }
        ...
        if (!data.einNumber || data.einNumber.trim() === '') {
          ...
          if (!einResult.success) {
            ctx.addIssue({
              code: z.ZodIssueCode.custom,
              message:
                einResult.error.errors[0]?.message || 'Invalid EIN format',
              path: ['einNumber'],
            })
          }
        }
    ...
    if (!data.mailingAddressSameAsRegistered) {
      ...
      if (
        needsState &&
        (!data.mailingState || data.mailingState.trim() === '')
      ) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
    ...
    if (!hasBillingAddress && !hasBillingStreet) {
      ...
      // `hasBillingStreet` already guarantees a non-empty string here.
      const billingStreet = data.billingStreet ?? ''
      // Validate billingStreet format if provided
      if (billingStreet.length < 10) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
      ...
      if (billingStreet.trim().length === 0) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
...
export const createOnboardingSchemaWithClassification = (
