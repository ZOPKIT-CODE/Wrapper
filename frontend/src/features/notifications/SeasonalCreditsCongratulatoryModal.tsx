  ...
  highlights: [
    {
      ...
      description: 'Access advanced tools and integrations',
    },
    {
      title: 'Usage Credits',
      description: 'Apply towards service consumption',
    },
  ],
  showActions: true,
...
export function mergeCongratulatoryDisplay(
  ...
  const cfg = {
    ...
    highlights: modalConfig?.highlights?.length
    ...
    actions: modalConfig?.actions?.length
      ? modalConfig.actions
      : DEFAULTS.actions,
...
export function SeasonalCreditsCongratulatoryPreview({
  ...
  return (
    <div className={className}>
      <Card
        className={`overflow-hidden border-0 shadow-xl ${cardClassName ?? ''}`}
      >
        <div
          className={`bg-gradient-to-br ${t.gradient} px-5 py-5 text-white sm:px-7 sm:py-6`}
        >
          ...
          <div className="flex items-end justify-between gap-2">
            <div>
              ...
              <p className="mt-0.5 text-sm text-white/60">Credits Added</p>
            </div>
            <Badge
              className={`${t.badgeBg} max-w-[140px] shrink-0 truncate border-0 px-3 py-1 text-xs text-white`}
            >
              {displayCampaign}
            </Badge>
          </div>
        ...
        <CardContent className="space-y-4 px-5 py-5 sm:space-y-5 sm:px-7 sm:py-6">
          <div className="text-center">
            <div
              className={`inline-flex h-12 w-12 items-center justify-center rounded-full sm:h-14 sm:w-14 ${t.checkBg} mb-3`}
            >
              <CheckCircle
                className={`h-6 w-6 sm:h-7 sm:w-7 ${t.checkColor}`}
              />
            </div>
          ...
          {cfg.showHighlights && cfg.highlights.length > 0 && (
            <div
              className={`grid gap-2 sm:gap-3 ${cfg.highlights.length === 1 ? 'grid-cols-1' : 'grid-cols-2'}`}
            >
              {cfg.highlights.slice(0, 4).map((h, i) => {
                ...
                return (
                  <div
                    key={i}
                    className={`rounded-xl border p-3 sm:p-4 ${t.highlightBg} ${t.highlightBorder}`}
                  >
          ...
          {cfg.showActions && cfg.actions.length > 0 && (
            <div
              className={`rounded-xl border p-3 sm:p-4 ${t.actionsBg} ${t.actionsBorder}`}
            >
              <h4
                className={`mb-2 flex items-center gap-2 text-sm font-semibold ${t.actionsTitle}`}
              >
              ...
              <ul className="space-y-1.5">
                {cfg.actions.map((action, i) => (
                  <li
                    key={i}
                    className={`flex items-start gap-2 text-xs sm:text-sm ${t.actionsText}`}
                  >
                    <span
                      className={`mt-1.5 h-1.5 w-1.5 shrink-0 rounded-full ${t.actionsBullet}`}
                    />
                    {action}
                  </li>
                ))}
          ...
          <div className="flex gap-2 pt-1 sm:gap-3">
            <Button
              ...
              className={`flex-1 text-sm font-medium text-white ${t.primaryBtn}`}
            >
              {cfg.primaryCta}
            </Button>
            <Button
              ...
              variant="outline"
              className="flex-1 border-gray-200 text-sm text-gray-700 hover:bg-gray-50"
            >
              {cfg.secondaryCta}
            </Button>
          </div>

          <p className="text-center text-[11px] leading-relaxed whitespace-pre-line text-gray-400 sm:text-xs">
            {cfg.footerNote}
          </p>
        </CardContent>
      </Card>
...
export const SeasonalCreditsCongratulatoryModal: React.FC<
  SeasonalCreditsCongratulatoryModalProps
> = ({
  isOpen,
  onClose,
  creditsAmount = 200,
  ...
  const display = mergeCongratulatoryDisplay(
    modalConfig,
    creditsAmount,
    campaignName
  )
  ...
  const handlePrimary = () => {
    onClose()
    navigate({ to: display.cfg.primaryCtaUrl as never })
  }
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm">
      <div className="relative w-full max-w-md">
        <button
          type="button"
          onClick={onClose}
          className="absolute -top-2 -right-2 z-20 flex h-8 w-8 items-center justify-center rounded-full border border-gray-200 bg-white text-gray-500 shadow-lg transition-colors hover:bg-gray-50"
          aria-label="Close"
        >
          <X className="h-4 w-4" />
        </button>
