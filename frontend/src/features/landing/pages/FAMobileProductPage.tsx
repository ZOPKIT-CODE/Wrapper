    ...
    return (
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', lineHeight: 1 }}>
                <div style={{ fontFamily: sans, fontWeight: 800, fontSize: 19, color: F.ink, letterSpacing: '-0.02em' }}>
                    Zopkit
                </div>
                <div style={{
                    fontFamily: sans, fontWeight: 500, fontSize: 6.5, color: F.ink, marginTop: 2,
                    border: `1px solid ${F.ink}`, padding: '1px 3px', borderRadius: 2,
...
function TopNav({ onMenu }: { onMenu: () => void }) {
    return (
        <div style={{ position: 'sticky', top: 0, zIndex: 50, padding: '12px 14px 8px', background: F.bg }}>
            <div style={{
                display: 'flex', alignItems: 'center', justifyContent: 'space-between',
                background: 'rgba(255,255,255,0.65)', backdropFilter: 'blur(20px)',
                border: '1px solid rgba(19,32,74,0.07)', borderRadius: 999,
...
// ─── Menu Overlay ─────────────────────────────────────────────
function Menu({ open, onClose, onNavigate }: { open: boolean; onClose: () => void; onNavigate: (to: string) => void }) {
    if (!open) return null;
    return (
        <div style={{
            position: 'absolute', inset: 0, zIndex: 100, background: F.bg,
            display: 'flex', flexDirection: 'column', padding: '14px 14px 24px',
        }}>
                ...
                {[
                    ...
                    <button
                        ...
                        style={{
                            ...
                            fontSize: 34, color: F.ink, textDecoration: 'none',
                            display: 'flex', justifyContent: 'space-between', alignItems: 'center',
                            background: 'none', border: 'none', borderBottom: `1px solid ${F.line}`,
    ...
    return (
        <div style={{ position: 'relative', padding: '20px 14px 0' }}>
            ...
            <div style={{ position: 'relative', marginTop: 20, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                ...
                <div className="mob-pp-float" style={{
                    position: 'relative', width: 160, height: 62, flexShrink: 0,
                    filter: 'drop-shadow(0 14px 22px rgba(0,0,0,0.55)) drop-shadow(0 0 38px rgba(27,46,90,0.45))',
                }}>
                    {/* Cylinder side */}
                    <div style={{
                    ...
                    {/* Ground shadow */}
                    <div style={{ position: 'absolute', bottom: '-14%', left: '18%', right: '18%', height: '18%', borderRadius: '50%', background: 'rgba(0,0,0,0.5)', filter: 'blur(12px)' }} />
                </div>
            </div>
        </div>
    );
    ...
    return (
        <div style={{ padding: '48px 0 32px', background: F.bg }}>
            <div style={{ textAlign: 'center', padding: '0 22px' }}>
                <div style={{ fontFamily: mono, fontSize: 11, color: F.blue, letterSpacing: '0.22em', textTransform: 'uppercase', fontWeight: 600 }}>
                    The Evolution
                </div>
                <h2 style={{ fontFamily: sans, fontWeight: 800, fontSize: 32, color: F.ink, margin: '12px 0 10px', letterSpacing: '-0.025em', lineHeight: 1.1 }}>
                    The Shift in Perspective
                </h2>
                <p style={{ fontFamily: sans, fontSize: 14, color: F.inkSoft, margin: 0, lineHeight: 1.55 }}>
                    See how Financial Accounting changes the game by bringing order to chaos.
                </p>
            </div>
            ...
            <div style={{ margin: '28px 14px 0', borderRadius: 16, overflow: 'hidden', background: 'linear-gradient(180deg, #FFF6F4 0%, #FFFFFF 100%)', border: '1px solid rgba(217,83,79,0.18)', position: 'relative' }}>
                ...
                <div style={{ padding: '20px 16px 22px' }}>
                    <h3 style={{ fontFamily: sans, fontWeight: 800, fontSize: 22, color: F.ink, margin: 0, letterSpacing: '-0.02em' }}>
    ...
    return (
        <div style={{ background: '#fff', borderRadius: full ? 14 : 10, overflow: 'hidden', boxShadow: '0 20px 40px -12px rgba(19,32,74,0.20), 0 0 0 1px rgba(19,32,74,0.04)' }}>
            ...
            <div style={{ display: 'flex' }}>
                    ...
                    <div style={{ width: 48, background: '#0B1430', display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '12px 0 12px', flexShrink: 0 }}>
                        ...
                        <div style={{ width: 30, height: 30, borderRadius: 7, background: '#3D7AE8', display: 'grid', placeItems: 'center', marginBottom: 14 }}>
                            <svg width="18" height="18" viewBox="0 0 18 18" fill="none">
                                ...
                                <rect x="2" y="10" width="6" height="6" rx="1.5" fill="rgba(255,255,255,0.45)" />
                                <rect x="10" y="10" width="6" height="6" rx="1.5" fill="rgba(255,255,255,0.25)" />
                            </svg>
                        </div>
                        {/* Other nav items */}
                        {Array.from({ length: 6 }).map((_, i) => (
                            <div key={i} style={{ width: 26, height: 3, borderRadius: 2, background: 'rgba(255,255,255,0.16)', marginBottom: 8 }} />
                        ))}
                        <div style={{ flex: 1 }} />
                        {/* User circle */}
                        <div style={{ width: 26, height: 26, borderRadius: 99, background: 'rgba(255,255,255,0.12)' }} />
                    </div>
                ) : (
                ...
                <div style={{ flex: 1, padding: full ? '12px 14px 16px' : '10px 10px 12px', overflow: 'hidden', minWidth: 0 }}>{children}</div>
            </div>
        </div>
    );
}

function MockHeader({ title, sub, cta, full }: { title: string; sub?: string; cta?: string; full?: boolean }) {
    return (
...
interface KPIItem { l: string; v: string; s?: string; sc?: string }
function KPIRow({ items, full }: { items: KPIItem[]; full?: boolean }) {
    const cols = full && items.length === 4 ? 2 : items.length;
    return (
        <div style={{ display: 'grid', gridTemplateColumns: `repeat(${cols}, 1fr)`, gap: full ? 10 : 5, marginTop: full ? 16 : 8 }}>
            {items.map((k, i) => (
                <div key={i} style={{ padding: full ? '14px 16px' : '6px 7px', borderRadius: full ? 8 : 5, background: F.bgSoft, border: `1px solid ${F.line}` }}>
                    <div style={{ fontFamily: mono, fontSize: full ? 10 : 6, color: F.muted, letterSpacing: '0.06em' }}>{k.l}</div>
                    <div style={{ fontFamily: sans, fontWeight: 700, fontSize: full ? 22 : 9, color: F.ink, marginTop: full ? 5 : 2, lineHeight: 1 }}>{k.v}</div>
                    {k.s && <div style={{ fontFamily: sans, fontSize: full ? 11 : 6.5, color: k.sc || F.muted, marginTop: full ? 3 : 1 }}>{k.s}</div>}
                </div>
            ))}
        </div>
    );
...
interface ColDef { h: string; w?: string; a?: 'left' | 'right' | 'center' }
function TableMock({ cols, rows, full }: { cols: ColDef[]; rows: CellVal[][]; full?: boolean }) {
    ...
    const inner = (
        <div>
            <div style={{ display: 'grid', gridTemplateColumns: tpl, gap, padding: full ? '8px 4px' : '4px 2px', fontFamily: mono, fontSize: hFs, color: F.muted, letterSpacing: '0.06em', borderBottom: `1px solid ${F.line}` }}>
                {cols.map((c, i) => <span key={i} style={{ textAlign: c.a || 'left' }}>{c.h}</span>)}
            </div>
            {rows.map((r, ri) => (
                <div key={ri} style={{ display: 'grid', gridTemplateColumns: tpl, gap, padding: full ? '12px 4px' : '5px 2px', fontFamily: sans, fontSize: rFs, color: F.ink, borderBottom: ri < rows.length - 1 ? `1px solid ${F.line}` : 'none', alignItems: 'center' }}>
                    {r.map((cell, ci) => {
                        const isObj = typeof cell === 'object';
                        return (
                            <span key={ci} style={{
                                textAlign: cols[ci].a || 'left',
                                fontFamily: isObj && (cell as any).mono ? mono : sans,
                                fontWeight: isObj && (cell as any).b ? 600 : 400,
                                color: isObj && (cell as any).c ? (cell as any).c : F.ink,
                            }}>
                                {isObj ? (cell as any).t : cell}
                            </span>
                        );
                    })}
                </div>
    ...
    return (
        <div style={{ marginTop: full ? 16 : 9, overflowX: 'hidden' }}>
            {inner}
        </div>
    );
}
...
function GLMock({ full }: { full?: boolean }) {
    const cols: ColDef[] = full
        ? [{ h: 'DATE', w: '0.7fr' }, { h: 'ENTRY #', w: '0.9fr' }, { h: 'ACCOUNT', w: '1fr' }, { h: 'DESCRIPTION', w: '1.4fr' }, { h: 'DEBIT', w: '0.9fr', a: 'right' }, { h: 'CREDIT', w: '0.9fr', a: 'right' }, { h: 'BALANCE', w: '0.9fr', a: 'right' }]
        : [{ h: 'DATE', w: '0.7fr' }, { h: 'ENTRY #', w: '0.9fr' }, { h: 'ACCOUNT', w: '1.1fr' }, { h: 'DEBIT', w: '0.9fr', a: 'right' }, { h: 'CREDIT', w: '0.9fr', a: 'right' }, { h: 'BALANCE', w: '0.9fr', a: 'right' }];
    const rows: CellVal[][] = full
        ? [
            ['Apr 26', { mono: true, b: true, c: F.blue, t: 'JE-0142' }, '1100 Cash', 'Customer receipt', { mono: true, t: '₹1,20,000' }, '—', { mono: true, c: F.green, b: true, t: '₹50,10,000' }],
            ['Apr 25', { mono: true, b: true, c: F.blue, t: 'JE-0141' }, '4000 Revenue', 'Sales invoice INV-22', '—', { mono: true, t: '₹3,40,000' }, { mono: true, c: F.green, b: true, t: '₹48,90,000' }],
            ['Apr 25', { mono: true, b: true, c: F.blue, t: 'JE-0140' }, '2100 AP', 'Vendor bill BL-7820', '—', { mono: true, t: '₹80,000' }, { mono: true, c: F.green, b: true, t: '₹48,10,000' }],
            ['Apr 24', { mono: true, b: true, c: F.blue, t: 'JE-0139' }, '6000 Salaries', 'April payroll', { mono: true, t: '₹2,10,000' }, '—', { mono: true, c: F.green, b: true, t: '₹50,20,000' }],
            ['Apr 24', { mono: true, b: true, c: F.blue, t: 'JE-0138' }, '3000 Capital', 'Owner contribution', '—', { mono: true, t: '₹5,00,000' }, { mono: true, c: F.green, b: true, t: '₹55,20,000' }],
          ]
        : [
            ['Apr 26', { mono: true, b: true, c: F.blue, t: 'JE-0142' }, '1100 Cash', { mono: true, t: '₹1,20,000' }, '—', { mono: true, c: F.green, b: true, t: '₹50,10,000' }],
            ['Apr 25', { mono: true, b: true, c: F.blue, t: 'JE-0141' }, '4000 Revenue', '—', { mono: true, t: '₹3,40,000' }, { mono: true, c: F.green, b: true, t: '₹48,90,000' }],
            ['Apr 25', { mono: true, b: true, c: F.blue, t: 'JE-0140' }, '2100 AP', '—', { mono: true, t: '₹80,000' }, { mono: true, c: F.green, b: true, t: '₹48,10,000' }],
            ['Apr 24', { mono: true, b: true, c: F.blue, t: 'JE-0139' }, '6000 Salaries', { mono: true, t: '₹2,10,000' }, '—', { mono: true, c: F.green, b: true, t: '₹50,20,000' }],
          ];
    return (
        <WinMock full={full}>
            <MockHeader title="General Ledger" sub="0 entries · Complete transaction history with running balances" cta="+ New JV" full={full} />
            ...
            <div style={{ marginTop: full ? 14 : 8, padding: full ? '10px 14px' : '6px 8px', borderRadius: full ? 6 : 4, background: 'rgba(77,193,138,0.10)', border: '1px solid rgba(77,193,138,0.25)', display: 'flex', justifyContent: 'space-between', fontFamily: sans, fontSize: full ? 12 : 8.5, color: F.green, fontWeight: 600 }}>
                <span>Trial Balance Reconciled ✓</span>
                <span style={{ color: F.muted, fontWeight: 400 }}>Debits = Credits · Apr 2026</span>
            </div>
        </WinMock>
    );
}
...
function EntityMock({ full }: { full?: boolean }) {
    if (full) {
        ...
        const registry: CellVal[][] = [
            [{ mono: true, b: true, t: 'GRP-HQ' }, 'Group HoldCo Ltd', 'Parent', 'INR', 'India', { c: F.green, b: true, t: '● Active' }],
            [{ mono: true, b: true, t: 'TCS-IN' }, 'TCS India Ltd', 'Subsidiary', 'INR', 'India', { c: F.green, b: true, t: '● Active' }],
            [{ mono: true, b: true, t: 'AcmeUS-LLC' }, 'Acme US LLC', 'Subsidiary', 'USD', 'US', { c: F.green, b: true, t: '● Active' }],
            [{ mono: true, b: true, t: 'AcmeSG-Pte' }, 'Acme SG Pte', 'Subsidiary', 'SGD', 'SG', { c: F.green, b: true, t: '● Active' }],
            [{ mono: true, b: true, t: 'AcmeUK-Ltd' }, 'Acme UK Ltd', 'Associate', 'GBP', 'UK', { c: F.muted, t: '● Inactive' }],
        ];
        return (
            <WinMock full>
                <MockHeader title="Entity Management" sub="Manage your group structure and consolidations" cta="+ Add Entity" full />
                <KPIRow full items={[
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginTop: 16 }}>
                    ...
                    <div style={{ borderRadius: 10, border: `1px solid ${F.line}`, background: F.bgSoft, padding: '16px 16px 14px' }}>
                        <div style={{ fontFamily: sans, fontWeight: 700, fontSize: 15, color: F.ink, marginBottom: 18 }}>Group Structure</div>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                            ...
                            <svg width="100%" height="42" style={{ display: 'block', overflow: 'visible' }} preserveAspectRatio="none">
                                <line x1="50%" y1="0" x2="50%" y2="18" stroke={F.line} strokeWidth="1.5" />
                                <line x1="16.5%" y1="18" x2="83.5%" y2="18" stroke={F.line} strokeWidth="1.5" />
                                <line x1="16.5%" y1="18" x2="16.5%" y2="42" stroke={F.line} strokeWidth="1.5" />
                                <line x1="50%" y1="18" x2="50%" y2="42" stroke={F.line} strokeWidth="1.5" />
                                <line x1="83.5%" y1="18" x2="83.5%" y2="42" stroke={F.line} strokeWidth="1.5" />
                            </svg>

                            {/* Children */}
                            <div style={{ display: 'flex', gap: 8, width: '100%' }}>
                                {subsidiaries.map((s) => (
                                    <div key={s.code} style={{ flex: 1, border: '1px solid rgba(61,122,232,0.28)', background: 'rgba(61,122,232,0.06)', borderRadius: 7, padding: '9px 6px', textAlign: 'center' }}>
                                        <div style={{ fontFamily: sans, fontSize: 12, fontWeight: 700, color: F.ink }}>{s.code}</div>
                                        <div style={{ fontFamily: sans, fontSize: 10.5, color: F.muted, marginTop: 3 }}>{s.sub}</div>
                                    </div>
                                ))}
                            </div>
                        </div>
                        ...
                        {/* Notes */}
                        <div style={{ marginTop: 14, padding: '8px 12px', borderRadius: 6, background: 'rgba(61,122,232,0.08)', fontFamily: sans, fontSize: 12, color: F.blue }}>
                            Equity method · Full consolidation
                        </div>
                        <div style={{ marginTop: 8, fontFamily: sans, fontSize: 12, color: F.inkSoft, padding: '2px 4px' }}>
                            Intercompany eliminations: 3 active
                        </div>
                        <div style={{ marginTop: 8, padding: '8px 12px', borderRadius: 6, background: 'rgba(77,193,138,0.10)', fontFamily: sans, fontSize: 12, color: F.green, fontWeight: 600 }}>
    ...
    return (
        <WinMock>
            <MockHeader title="Entity Management" sub="Manage your group structure and consolidations" cta="+ Add Entity" />
            <KPIRow items={[
                { l: 'TOTAL ENTITIES', v: '6', s: 'All registered' },
                { l: 'ACTIVE', v: '6', s: 'Currently running' },
                { l: 'SUBSIDIARIES', v: '4', s: 'Under group' },
            ...
            <div style={{ marginTop: 9, padding: 8, borderRadius: 5, background: F.bgSoft, border: `1px solid ${F.line}` }}>
                ...
                <div style={{ marginTop: 6, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
                    ...
                    <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', justifyContent: 'center' }}>
                        {['IN', 'US', 'SG'].map((c) => (
                            <div key={c} style={{ border: '1px solid rgba(61,122,232,0.3)', background: 'rgba(61,122,232,0.08)', borderRadius: 4, padding: '4px 6px', textAlign: 'center' }}>
                                <div style={{ fontFamily: sans, fontSize: 7, fontWeight: 600, color: F.ink }}>{c}-Subsidiary</div>
                            </div>
                        ))}
                    </div>
                </div>
                <div style={{ marginTop: 8, padding: '4px 6px', borderRadius: 3, background: 'rgba(77,193,138,0.12)', fontFamily: sans, fontSize: 8, color: F.green, fontWeight: 600 }}>Dec 2025 consolidation: complete ✓</div>
            </div>
            <TableMock
                cols={[{ h: 'CODE', w: '0.8fr' }, { h: 'ENTITY', w: '1.4fr' }, { h: 'TYPE', w: '1fr' }, { h: 'CUR', w: '0.6fr' }, { h: 'STATUS', w: '0.9fr', a: 'right' }]}
                rows={[
                    [{ mono: true, b: true, t: 'GRP-HQ' }, 'Group HoldCo Ltd', 'Parent', 'INR', { c: F.green, b: true, t: 'Active' }],
                    [{ mono: true, b: true, t: 'TCS-IN' }, 'TCS India Ltd', 'Subsidiary', 'INR', { c: F.green, b: true, t: 'Active' }],
                    [{ mono: true, b: true, t: 'AcmeUS' }, 'Acme US LLC', 'Subsidiary', 'USD', { c: F.green, b: true, t: 'Active' }],
                ]}
            />
        </WinMock>
    );
...
function FXMock({ full }: { full?: boolean }) {
    ...
    return (
        <WinMock full={full}>
            <MockHeader title="Multi-Currency" sub="Currency settings and FX rate management" full={full} />
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: full ? 10 : 6, marginTop: full ? 14 : 8 }}>
                <div style={{ padding: full ? '12px 14px' : '7px 8px', borderRadius: full ? 8 : 5, background: 'rgba(61,122,232,0.08)', border: '1px solid rgba(61,122,232,0.20)' }}>
                    <div style={{ fontFamily: mono, fontSize: full ? 10 : 6.5, color: F.blue, letterSpacing: '0.06em' }}>FUNCTIONAL CURRENCY</div>
                    <div style={{ fontFamily: sans, fontSize: full ? 14 : 9.5, fontWeight: 700, color: F.ink, marginTop: full ? 4 : 1 }}>₹ INR — Indian Rupee</div>
                </div>
                <div style={{ padding: full ? '12px 14px' : '7px 8px', borderRadius: full ? 8 : 5, background: F.bgSoft, border: `1px solid ${F.line}` }}>
                    <div style={{ fontFamily: mono, fontSize: full ? 10 : 6.5, color: F.muted, letterSpacing: '0.06em' }}>RATE SOURCE</div>
                    <div style={{ fontFamily: sans, fontSize: full ? 13 : 9, fontWeight: 600, color: F.ink, marginTop: full ? 4 : 1 }}>RBI Reference Rate</div>
                </div>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: full ? 10 : 5, marginTop: full ? 10 : 7 }}>
                {rates.map((x) => (
                    <div key={x.c} style={{ padding: full ? '12px 14px' : '6px 7px', borderRadius: full ? 8 : 5, background: F.bgSoft, border: `1px solid ${F.line}` }}>
                        <div style={{ fontFamily: sans, fontWeight: 700, fontSize: full ? 13 : 8.5, color: F.inkSoft }}>{x.c}</div>
                        <div style={{ fontFamily: sans, fontWeight: 800, fontSize: full ? 18 : 10, color: F.ink, marginTop: full ? 4 : 1 }}>{x.v}</div>
                        <div style={{ fontFamily: sans, fontSize: full ? 11 : 7, color: x.dn ? F.red : F.green }}>{x.d}</div>
                    </div>
                ))}
            </div>
            <TableMock full={full}
                cols={[{ h: 'DATE', w: '0.7fr' }, { h: 'DOC', w: '1fr' }, { h: 'FROM→TO', w: '0.9fr' }, { h: 'RATE', w: '0.6fr', a: 'right' }, { h: 'AMT (₹)', w: '1fr', a: 'right' }, { h: 'G/L', w: '0.7fr', a: 'right' }]}
                rows={[
                    ['Apr 26', { mono: true, b: true, c: F.ink, t: 'INV-000022' }, { mono: true, t: 'USD→INR' }, { mono: true, t: '83.45' }, { mono: true, t: '₹4,91,955' }, { mono: true, b: true, c: F.green, t: '+₹840' }],
                    ['Apr 24', { mono: true, b: true, c: F.ink, t: 'BL-7820' }, { mono: true, t: 'USD→INR' }, { mono: true, t: '83.38' }, { mono: true, t: '₹2,83,492' }, { mono: true, b: true, c: F.green, t: '+₹240' }],
                    ['Apr 22', { mono: true, b: true, c: F.ink, t: 'INV-000019' }, { mono: true, t: 'EUR→INR' }, { mono: true, t: '89.08' }, { mono: true, t: '₹2,20,728' }, { mono: true, b: true, c: F.red, t: '−₹360' }],
                ]}
            />
            <div style={{ display: 'inline-flex', marginTop: full ? 14 : 8, padding: full ? '7px 12px' : '4px 8px', borderRadius: full ? 6 : 4, background: 'rgba(77,193,138,0.14)', fontFamily: sans, fontSize: full ? 12 : 8.5, color: F.green, fontWeight: 600 }}>
...
function APMock({ full }: { full?: boolean }) {
    return (
        <WinMock full={full}>
            <MockHeader title="Bills · TCS · Apr 2026" sub="AP · Vendor bills, 3-way matching, and payment approvals" cta="+ New Bill" full={full} />
            <KPIRow full={full} items={[
...
function ARMock({ full }: { full?: boolean }) {
    return (
        <WinMock full={full}>
            <MockHeader title="Customer Invoices" sub="19 INVOICES · AR · Customer invoice history and receivables" cta="+ New Invoice" full={full} />
            <KPIRow full={full} items={[
...
function BankMock({ full }: { full?: boolean }) {
    ...
    return (
        <WinMock full={full}>
            <MockHeader title="Banking Dashboard" sub="Monitor bank accounts, transactions, and reconciliations" full={full} />
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: full ? 10 : 5, marginTop: full ? 14 : 8 }}>
                {banks.map((b) => (
                    <div key={b.n} style={{ padding: full ? '12px 14px' : '6px 7px', borderRadius: full ? 8 : 5, background: F.bgSoft, border: `1px solid ${F.line}` }}>
                        <div style={{ fontFamily: sans, fontWeight: 700, fontSize: full ? 13 : 8.5, color: F.ink }}>{b.n} Bank</div>
                        <div style={{ fontFamily: mono, fontSize: full ? 10 : 6.5, color: F.muted }}>{b.a}</div>
                        <div style={{ fontFamily: sans, fontWeight: 700, fontSize: full ? 18 : 9.5, color: F.ink, marginTop: full ? 4 : 2 }}>{b.v}</div>
                        <div style={{ fontFamily: sans, fontSize: full ? 10 : 6.5, color: b.sc, marginTop: full ? 2 : 1 }}>{b.s}</div>
                    </div>
                ))}
            </div>
            <TableMock full={full}
                cols={[{ h: 'DATE', w: '0.6fr' }, { h: 'DESCRIPTION', w: '1.4fr' }, { h: 'RULE', w: '1fr' }, { h: 'AMT', w: '1fr', a: 'right' }, { h: 'MATCH', w: '0.8fr', a: 'right' }]}
                rows={[
...
function FAAssetsMock({ full }: { full?: boolean }) {
    return (
        <WinMock full={full}>
            <MockHeader title="Fixed Assets" sub="Asset register, depreciation, and asset lifecycle management" cta="+ Add Asset" full={full} />
            <KPIRow full={full} items={[
            ...
            <TableMock full={full}
                ...
                rows={[
                    [{ mono: true, b: true, t: 'FA-0042' }, 'Dell PowerEdge R750', 'IT Hardware', 'SLM', { mono: true, t: '₹3,40,000' }],
                    [{ mono: true, b: true, t: 'FA-0089' }, 'Office Lease HSR', 'Buildings', 'SLM', { mono: true, t: '₹78,00,000' }],
                    [{ mono: true, b: true, t: 'FA-0114' }, 'Toyota Innova 2023', 'Vehicles', 'DBM', { mono: true, t: '₹6,20,000' }],
...
// ─── Mock Expand Modal ────────────────────────────────────────
function MockExpandModal({ title, children, onClose }: { title: string; children: React.ReactNode; onClose: () => void }) {
    useEffect(() => {
    ...
    return (
        ...
        >
            ...
            <div style={{ flexShrink: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '6px 0 18px', background: 'rgba(8,12,28,0.96)' }}>
                {/* Beam */}
                <div style={{
                    width: 80, height: 70,
                    background: 'linear-gradient(to top, rgba(46,79,140,0.55) 0%, rgba(60,100,200,0.22) 40%, transparent 100%)',
                    filter: 'blur(8px)', borderRadius: '50% 50% 0 0',
                    marginBottom: -34, pointerEvents: 'none',
                }} />
                {/* 3D puck at ~60% size */}
                <div className="mob-pp-float" style={{
                    position: 'relative', width: 96, height: 38, flexShrink: 0,
                    filter: 'drop-shadow(0 8px 14px rgba(0,0,0,0.55)) drop-shadow(0 0 24px rgba(27,46,90,0.45))',
                }}>
                    {/* Cylinder side */}
                    <div style={{
                        position: 'absolute', top: '20%', left: 0, right: 0, bottom: 0,
...
function FeaturePanel({ num, bg, accent, icon, title, desc, bullets, chip, children, first, last, onExpand }: FeaturePanelProps) {
    return (
        <div style={{
            padding: '28px 18px 26px',
            borderRadius: first ? '22px 22px 0 0' : (last ? '22px 22px 22px 22px' : '22px 22px 0 0'),
            ...
            borderTop: first ? '1px solid rgba(19,32,74,0.06)' : '1px solid rgba(19,32,74,0.10)',
            marginTop: first ? 0 : -14,
        }}>
            {!first && (
                <div style={{ position: 'absolute', top: 0, left: 16, right: 16, height: 1, background: 'rgba(255,255,255,0.55)' }} />
            )}
            ...
            >
                ...
                <div style={{
                    ...
                    background: 'rgba(19,32,74,0.75)', backdropFilter: 'blur(6px)',
                    borderRadius: 999, padding: '5px 10px',
                    pointerEvents: 'none',
                }}>
                    <svg width="11" height="11" viewBox="0 0 11 11" fill="none">
                        <path d="M1 10L10 1M10 1H5M10 1V6" stroke="rgba(255,255,255,0.85)" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
                    </svg>
                    <span style={{ fontFamily: mono, fontSize: 9, color: 'rgba(255,255,255,0.85)', letterSpacing: '0.08em', fontWeight: 600 }}>VIEW FULL</span>
                </div>
            </div>

            {/* ── Feature label ── */}
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, fontFamily: mono, fontSize: 10, letterSpacing: '0.22em', color: accent, fontWeight: 700 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginTop: 14 }}>
                <div style={{ width: 44, height: 44, borderRadius: 12, background: 'rgba(255,255,255,0.7)', display: 'grid', placeItems: 'center', color: accent, flexShrink: 0, boxShadow: '0 2px 8px rgba(19,32,74,0.08)' }}>
                    {icon}
                </div>
                <h2 style={{ fontFamily: "'Inter', system-ui, sans-serif", fontWeight: 800, fontSize: 26, lineHeight: 1.1, color: F.ink, margin: 0, letterSpacing: '-0.03em' }}>
                    {title}
                </h2>
            </div>

            {/* ── Description ── */}
            <p style={{ fontFamily: "'Inter', system-ui, sans-serif", fontSize: 14, color: F.inkSoft, margin: '12px 0 0', lineHeight: 1.6, fontWeight: 400 }}>
                {desc}
            </p>
            ...
            <ul style={{ listStyle: 'none', padding: 0, margin: '16px 0 0', display: 'flex', flexDirection: 'column', gap: 10 }}>
                {bullets.map((c, i) => (
                    <li key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
                        <svg width="16" height="16" viewBox="0 0 16 16" style={{ marginTop: 2, flex: '0 0 16px' }}>
                            <circle cx="8" cy="8" r="7" fill={accent} opacity={0.12} />
                            <path d="M4.5 8l2.5 2.5 4.5-5" stroke={accent} strokeWidth="1.7" fill="none" strokeLinecap="round" strokeLinejoin="round" />
                        </svg>
                        <span style={{ fontFamily: "'Inter', system-ui, sans-serif", fontSize: 13.5, color: F.inkSoft, lineHeight: 1.5, fontWeight: 450 }}>{c}</span>
                    </li>
                ))}
            </ul>
...
function CapabilitiesSection({ onExpand }: { onExpand: (title: string, node: React.ReactNode) => void }) {
    const features: FeaturePanelProps[] = [
        {
            ...
            bullets: ['Custom chart of accounts with multi-level hierarchy', 'Journal entry creation with multi-level approval workflows', 'Period-lock controls with admin-only override', 'Real-time trial balance and posting validation'],
            chip: { label: 'Accuracy', value: '99.9%' },
            children: <GLMock />, fullChildren: <GLMock full />,
        },
        {
            ...
            bullets: ['Independent chart of accounts per entity, shared master optional', 'Server-side consolidation across any entity combination', 'Inter-company transaction posting and automatic elimination', 'Role-based access scoped per entity', 'Consolidated P&L and balance sheet at group level'],
            chip: { label: 'Time saved', value: '40h/mo' },
            children: <EntityMock />, fullChildren: <EntityMock full />,
        },
        {
            ...
            children: <FXMock />, fullChildren: <FXMock full />,
        },
        {
            ...
            children: <APMock />, fullChildren: <APMock full />,
        },
        {
            ...
            bullets: ['Customer invoicing with CGST/SGST/IGST line items', 'Aging report with configurable buckets (0-30, 31-60, 61-90, 90+)', 'Automated payment reminder scheduling', 'Receipt application against open invoices and advance management', 'Customer-level credit limit enforcement'],
            chip: { label: 'Entries saved', value: '−60%' },
            children: <ARMock />, fullChildren: <ARMock full />,
        },
        {
            ...
            bullets: ['Multi-bank account register with running balance', 'Bank statement import via CSV or OFX', 'Rule-based transaction matching engine', 'Cash position dashboard across all banks', 'Reconciliation workflow with exception handling'],
            chip: { label: 'Time saved', value: '40h/mo' },
            children: <BankMock />, fullChildren: <BankMock full />,
        },
        {
            ...
            children: <FAAssetsMock />, fullChildren: <FAAssetsMock full />,
        },
    ];

    return (
        <>
            <div style={{ padding: '56px 22px 8px', textAlign: 'center', background: F.bg }}>
                <div style={{ fontFamily: mono, fontSize: 11, color: F.muted, letterSpacing: '0.28em', textTransform: 'uppercase', fontWeight: 500 }}>CAPABILITIES</div>
                <h2 style={{ fontFamily: sans, fontWeight: 800, fontSize: 38, color: F.ink, margin: '14px 0 10px', letterSpacing: '-0.028em', lineHeight: 1.05 }}>Everything you need</h2>
                <p style={{ fontFamily: sans, fontSize: 14, color: F.inkSoft, margin: 0, lineHeight: 1.55 }}>
                    Explore the powerful capabilities built into the core of Financial Accounting.
                </p>
            </div>
...
function IndustrySection() {
    const segs = [
        { t: 'Small Businesses', d: 'Simple accounting for small businesses with automated workflows and tax compliance.', b: ['Easy bookkeeping', 'Automated reconciliation', 'Financial reports', 'Tax compliance', 'Mobile access'] },
        { t: 'Growing Companies', d: 'Scalable accounting for companies with complex financial needs and multi-currency operations.', b: ['Multi-entity support', 'Advanced reporting', 'Workflow automation', 'Integration ready', 'Cost accounting'], featured: true },
        { t: 'Enterprises', d: 'Enterprise-grade accounting with advanced features, controls, and multi-entity consolidation.', b: ['Multi-currency', 'Consolidation', 'Advanced security', 'Custom workflows', 'Audit compliance'] },
        { t: 'Multi-Location Businesses', d: 'Consolidated accounting across multiple locations with centralized control and local compliance.', b: ['Branch accounting', 'Location-wise reports', 'Inter-branch transfers', 'Local tax compliance'] },
        { t: 'Holding Companies', d: 'Multi-entity consolidation and reporting for holding companies with inter-company transaction management.', b: ['Group consolidation', 'Inter-company eliminations', 'Investment tracking', 'Group-level reports'] },
    ];
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginTop: 28 }}>
                {segs.map((s) => (
                    <div key={s.t} style={{
                        ...
                        background: s.featured ? 'linear-gradient(180deg, rgba(61,122,232,0.18), rgba(61,122,232,0.06))' : 'rgba(255,255,255,0.025)',
    ...
    return (
        <div style={{ background: F.bg, padding: '56px 14px 8px' }}>
            <div style={{ textAlign: 'left', padding: '0 4px 22px' }}>
                <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, fontFamily: mono, fontSize: 10, color: F.muted, letterSpacing: '0.28em', fontWeight: 600 }}>
                    <span style={{ width: 18, height: 1, background: F.muted }} />
                    IN PRACTICE
                </div>
                <h2 style={{ fontFamily: sans, fontWeight: 800, fontSize: 34, color: F.ink, margin: '12px 0 0', letterSpacing: '-0.03em', lineHeight: 1.02 }}>
                    What teams find{' '}
                    <span style={{ fontFamily: serif, fontStyle: 'italic', fontWeight: 500 }}>after switching.</span>
                </h2>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column' }}>
                {rows.map((r, i) => (
                    <div key={i} style={{
                        ...
                        borderBottom: i === rows.length - 1 ? `1px solid ${F.ink}` : 'none',
                        gap: 14,
                    }}>
                        <div style={{ fontFamily: sans, fontWeight: 800, fontSize: 42, color: F.ink, letterSpacing: '-0.035em', lineHeight: 0.95, flex: '0 0 auto' }}>
                            {r.v}<span style={{ fontSize: 18, color: F.muted, fontWeight: 600 }}>{r.u}</span>
                        </div>
                        <div style={{ fontFamily: sans, fontSize: 13, color: F.inkSoft, textAlign: 'right', lineHeight: 1.4, maxWidth: 200 }}>
                            {r.l}
                        </div>
                    </div>
                ))}
...
function ComparePlans() {
    const modules = [
        ['AI insights', 0, 0, 1], ['Analytics', 1, 1, 1], ['Banking', 1, 1, 1],
        ['Bills', 1, 1, 1], ['Budgeting', 0, 1, 1], ['Chart of accounts', 1, 1, 1],
        ['Compliance', 0, 1, 1], ['Customers', 1, 1, 1], ['Dashboard', 1, 1, 1],
        ['Documents', 1, 1, 1], ['Estimates', 1, 1, 1], ['Expense reports', 1, 1, 1],
        ['Fixed assets', 0, 1, 1], ['General ledger', 1, 1, 1], ['Integrations', 0, 1, 1],
        ['Inventory', 0, 1, 1], ['Invoices', 1, 1, 1], ['Multi-currency', 0, 1, 1],
        ['Multi-entity', 0, 0, 1], ['Payroll', 0, 1, 1], ['Receipts', 1, 1, 1],
        ['Reports', 1, 1, 1], ['Tax', 1, 1, 1], ['Vendors', 1, 1, 1],
    ];

    function Check() {
        return (
            <div style={{ width: 22, height: 22, borderRadius: 99, background: 'rgba(61,122,232,0.10)', display: 'grid', placeItems: 'center', margin: '0 auto' }}>
                <svg width="12" height="12" viewBox="0 0 12 12" fill="none">
                    <path d="M2 6l3 3 5-6" stroke={F.blue} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                </svg>
            </div>
        );
    }
    ...
    return (
        <div style={{ background: F.bg, padding: '56px 0 36px' }}>
            <div style={{ textAlign: 'center', padding: '0 22px' }}>
                <div style={{ display: 'inline-flex', alignItems: 'center', gap: 7, padding: '7px 14px', borderRadius: 99, background: 'rgba(61,122,232,0.10)' }}>
                    <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
                        <rect x="1" y="1" width="5" height="5" rx="1" stroke={F.blue} strokeWidth="1.6" />
                        <rect x="8" y="1" width="5" height="5" rx="1" stroke={F.blue} strokeWidth="1.6" />
                        <rect x="1" y="8" width="5" height="5" rx="1" stroke={F.blue} strokeWidth="1.6" />
                        <rect x="8" y="8" width="5" height="5" rx="1" stroke={F.blue} strokeWidth="1.6" />
                    </svg>
                    <span style={{ fontFamily: mono, fontSize: 10.5, color: F.blue, letterSpacing: '0.18em', fontWeight: 700 }}>COMPARE PLANS</span>
                </div>
                <h2 style={{ fontFamily: sans, fontWeight: 800, fontSize: 34, color: F.ink, margin: '18px 0 0', letterSpacing: '-0.028em', lineHeight: 1.05 }}>
                    Modules by plan
                </h2>
            </div>
                ...
                <div style={{ background: '#fff', border: `1px solid ${F.line}`, borderRadius: 16, overflow: 'hidden', boxShadow: '0 8px 24px -8px rgba(19,32,74,0.08)' }}>
                    <div style={{ display: 'grid', gridTemplateColumns: '1.4fr 0.9fr 0.9fr 0.9fr', background: F.bgSoft, borderBottom: `1px solid ${F.line}` }}>
                        ...
                        {['Starter', 'Professional', 'Enterprise'].map((p) => (
                            <div key={p} style={{ padding: '14px 6px', textAlign: 'center', borderLeft: `1px solid ${F.line}` }}>
                                <div style={{ fontFamily: sans, fontWeight: 800, fontSize: 13, color: F.ink, letterSpacing: '-0.01em' }}>{p}</div>
                                <div style={{ fontFamily: sans, fontSize: 8.5, color: F.muted, marginTop: 2 }}>Annual</div>
                            </div>
                        ))}
                    </div>
                    {modules.map((row, i) => (
                        <div key={i} style={{ display: 'grid', gridTemplateColumns: '1.4fr 0.9fr 0.9fr 0.9fr', borderBottom: i < modules.length - 1 ? `1px solid ${F.line}` : 'none', background: i % 2 ? '#fff' : 'rgba(19,32,74,0.015)', alignItems: 'center' }}>
                            ...
                            {[1, 2, 3].map((j) => (
                                <div key={j} style={{ padding: '12px 4px', textAlign: 'center', borderLeft: `1px solid ${F.line}` }}>
                                    {row[j] ? <Check /> : <span style={{ color: F.muted, fontFamily: sans, fontSize: 14 }}>—</span>}
                                </div>
                            ))}
                        </div>
                    ))}
    ...
    return (
        <div style={{ background: '#0A1024', padding: '40px 18px 100px', color: '#fff' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24, marginTop: 28 }}>
                {sections.map((c) => (
                    <div key={c.h}>
                        <div style={{ fontFamily: sans, fontSize: 11, fontWeight: 700, color: 'rgba(255,255,255,0.9)', letterSpacing: '0.12em', textTransform: 'uppercase' }}>{c.h}</div>
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 9, marginTop: 12 }}>
                            {c.l.map((l) => (
                                <button
                                    key={l}
                                    onClick={() => onNavigate('/landing')}
                                    style={{ fontFamily: sans, fontSize: 13, color: 'rgba(255,255,255,0.55)', textDecoration: 'none', background: 'none', border: 'none', cursor: 'pointer', textAlign: 'left', padding: 0 }}
                                >
                                    {l}
                                </button>
                            ))}
            ...
            <div style={{ marginTop: 32, paddingTop: 18, borderTop: '1px solid rgba(255,255,255,0.08)', fontFamily: sans, fontSize: 11, color: 'rgba(255,255,255,0.4)' }}>
...
// ─── Sticky CTA ───────────────────────────────────────────────
function StickyCTA({ visible, onTrial }: { visible: boolean; onTrial: () => void }) {
    return (
        <div style={{
...
export function FAMobileProductPage() {
    ...
    const [expandedMock, setExpandedMock] = useState<{ title: string; node: React.ReactNode } | null>(null);
    ...
    return (
        <div style={{ width: '100%', height: '100dvh', position: 'relative', overflow: 'hidden', background: F.bg }}>
            <div
                ref={scrollRef}
                style={{ width: '100%', height: '100%', overflowY: 'auto', overflowX: 'hidden', WebkitOverflowScrolling: 'touch' } as React.CSSProperties}
            >
                <TopNav onMenu={() => setMenuOpen(true)} />
                <HeroSection />
                <ShiftSection />
                <CapabilitiesSection onExpand={(title, node) => setExpandedMock({ title, node })} />
                <IndustrySection />
                <NumbersBand />
                <ComparePlans />
                <Footer onNavigate={handleNavigate} />
            </div>
            <StickyCTA visible={showCTA && !menuOpen && !expandedMock} onTrial={() => handleNavigate('/onboarding')} />
            <Menu open={menuOpen} onClose={() => setMenuOpen(false)} onNavigate={handleNavigate} />
            {/* Modal rendered here — outside scroll div — so position:fixed works on all mobile browsers */}
            {expandedMock && (
                <MockExpandModal title={expandedMock.title} onClose={() => setExpandedMock(null)}>
                    {expandedMock.node}
                </MockExpandModal>
            )}
        </div>
