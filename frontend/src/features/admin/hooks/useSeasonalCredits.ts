...
import { seasonalCreditsAPI, seasonalCreditBatchesAPI } from '@/lib/api/seasonal-credits';

// ─── Query Keys ───────────────────────────────────────────────────────────────

export const seasonalCreditKeys = {
  all: ['admin', 'seasonal-credits'] as const,
  campaigns: (filters?: object) => [...seasonalCreditKeys.all, 'campaigns', filters] as const,
  campaign: (id: string) => [...seasonalCreditKeys.all, 'campaign', id] as const,
  campaignStatus: (id: string) => [...seasonalCreditKeys.all, 'campaign-status', id] as const,
  campaignTenantBreakdown: (id: string) => [...seasonalCreditKeys.all, 'campaign-tenant-breakdown', id] as const,
  expiringSoon: (days?: number) => [...seasonalCreditKeys.all, 'expiring-soon', days] as const,
  activeBatches: (filters?: object) => [...seasonalCreditKeys.all, 'batches', 'active', filters] as const,
  expiredHistory: (filters?: object) => [...seasonalCreditKeys.all, 'batches', 'expired', filters] as const,
  tenantSummary: (tenantId: string) => [...seasonalCreditKeys.all, 'tenant', tenantId] as const,
  cronStatus: (limit?: number) => [...seasonalCreditKeys.all, 'cron-status', limit] as const,
};

// ─── Campaign Queries ─────────────────────────────────────────────────────────

export function useCampaigns(filters?: { isActive?: boolean; distributionStatus?: string }) {
  return useQuery({
...
export function useCampaignTenantBreakdown(campaignId: string) {
  return useQuery({
    ...
    queryFn: async () => {
      const res = await seasonalCreditsAPI.getCampaignTenantBreakdown(campaignId);
...
export function useDistributeCampaign() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (campaignId: string) => seasonalCreditsAPI.distributeCampaign(campaignId),
    onSuccess: (_data, campaignId) => {
      qc.invalidateQueries({ queryKey: seasonalCreditKeys.campaign(campaignId) });
      qc.invalidateQueries({ queryKey: seasonalCreditKeys.campaigns() });
      qc.invalidateQueries({ queryKey: seasonalCreditKeys.campaignTenantBreakdown(campaignId) });
...
export function useExtendCampaign() {
  ...
  return useMutation({
    ...
    onError: (err: Error) => toast.error(err.message ?? 'Failed to extend campaign'),
  });
}
...
export function useCancelCampaign() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (campaignId: string) => seasonalCreditsAPI.cancelCampaign(campaignId),
    onSuccess: (_data, campaignId) => {
      qc.invalidateQueries({ queryKey: seasonalCreditKeys.all });
      qc.invalidateQueries({ queryKey: seasonalCreditKeys.campaignTenantBreakdown(campaignId) });
...
export function useRerunFailed() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (campaignId: string) => seasonalCreditsAPI.rerunFailed(campaignId),
    onSuccess: (_data, campaignId) => {
      qc.invalidateQueries({ queryKey: seasonalCreditKeys.campaign(campaignId) });
      qc.invalidateQueries({ queryKey: seasonalCreditKeys.campaigns() });
      qc.invalidateQueries({ queryKey: seasonalCreditKeys.campaignTenantBreakdown(campaignId) });
...
export function useGrantToTenant() {
  ...
  return useMutation({
    ...
    onSuccess: (_data, { tenantId }) => {
      qc.invalidateQueries({ queryKey: seasonalCreditKeys.tenantSummary(tenantId) });
