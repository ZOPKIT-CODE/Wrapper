...
export function CreateCampaignForm({
  ...
  const setMC = <K extends keyof typeof EMPTY_MODAL_CONFIG>(
    key: K,
    val: (typeof EMPTY_MODAL_CONFIG)[K]
  ) => setModalConfig((prev) => ({ ...prev, [key]: val }))

  const setHighlight = (i: number, field: 'title' | 'description', v: string) =>
    setModalConfig((prev) => ({
      ...prev,
      highlights: prev.highlights.map((h, idx) =>
        idx === i ? { ...h, [field]: v } : h
      ),
    }))
  ...
  const setAction = (i: number, v: string) =>
    setModalConfig((prev) => ({
      ...prev,
      actions: prev.actions.map((a, idx) => (idx === i ? v : a)),
    }))

  const addAction = () =>
    setModalConfig((prev) => ({ ...prev, actions: [...prev.actions, ''] }))
  ...
  const buildCleanConfig = (): ModalConfig | undefined => {
    ...
    if (modalConfig.headerTitle.trim())
      c.headerTitle = modalConfig.headerTitle.trim()
    if (modalConfig.headerSubtitle.trim())
      c.headerSubtitle = modalConfig.headerSubtitle.trim()
    if (modalConfig.headline.trim()) c.headline = modalConfig.headline.trim()
    if (modalConfig.description.trim())
      c.description = modalConfig.description.trim()
    if (!modalConfig.showHighlights) c.showHighlights = false
    const cleanH = modalConfig.highlights.filter((h) => h.title.trim())
    if (cleanH.length)
      c.highlights = cleanH.map((h) => ({
        title: h.title.trim(),
        description: h.description.trim(),
      }))
    if (!modalConfig.showActions) c.showActions = false
    if (modalConfig.actionsTitle.trim())
      c.actionsTitle = modalConfig.actionsTitle.trim()
    const cleanA = modalConfig.actions.filter((a) => a.trim())
    if (cleanA.length) c.actions = cleanA.map((a) => a.trim())
    if (modalConfig.primaryCta.trim())
      c.primaryCta = modalConfig.primaryCta.trim()
    if (modalConfig.secondaryCta.trim())
      c.secondaryCta = modalConfig.secondaryCta.trim()
    if (modalConfig.footerNote.trim())
      c.footerNote = modalConfig.footerNote.trim()
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const payload: Parameters<typeof create.mutate>[0] = {
      ...
      creditsPerTenant: form.creditsPerTenant
        ? Number(form.creditsPerTenant)
        : undefined,
  ...
  const previewExpiry =
    devMode && Number(devMinutes) > 0 ? `~${devMinutes} min` : null
  ...
  const previewConfig: ModalConfig = {
    ...
    actions: modalConfig.actions.some(Boolean)
      ? modalConfig.actions.filter(Boolean)
      : undefined,
  ...
  const previewCredits = form.creditsPerTenant
    ? Number(form.creditsPerTenant)
    : form.totalCredits
      ? Number(form.totalCredits)
      : 500
  const previewCampaignName = form.campaignName || 'Campaign Preview'
  const congratulatoryDisplay = mergeCongratulatoryDisplay(
    previewConfig,
    previewCredits,
    previewCampaignName
  )
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      {IS_DEV && (
        <div className="flex items-center gap-2 rounded-md border border-amber-200 bg-amber-50 px-3 py-2 text-sm">
          ...
          <span className="flex-1 text-amber-700">Dev mode — short expiry</span>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <div className="space-y-2">
          ...
          <Input
            ...
            value={form.creditsPerTenant}
            onChange={(e) => set('creditsPerTenant')(e.target.value)}
          />
        </div>
        <div className="space-y-2">
          {devMode ? (
      ...
      <div className="flex items-center gap-2">
        <Switch
          ...
          onCheckedChange={(v) => set('targetAllTenants')(v)}
        />
      ...
      <div className="flex items-center gap-2">
        <Switch
          ...
          onCheckedChange={(v) => set('notifyTenants')(v)}
        />
        ...
        <div className="overflow-hidden rounded-xl border border-slate-200 bg-white">
          <div className="border-b border-slate-200 bg-slate-50 px-4 py-3">
            ...
            <p className="text-muted-foreground mt-0.5 text-xs">
              Customize the message on the left; the live preview updates on the
              right.
            </p>
          </div>

          <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_minmax(280px,380px)] lg:items-start">
            <div className="max-h-[min(75vh,880px)] min-w-0 divide-y divide-slate-100 overflow-y-auto border-b border-slate-200 lg:border-r lg:border-b-0">
                ...
                <Section title="Theme">
                  <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
                    {THEMES.map((th) => (
                      ...
                      >
                        <div
                          className={`bg-gradient-to-br ${th.gradient} h-10 w-full`}
                        />
                        ...
                        {modalConfig.theme === th.value && (
                          <div className="absolute top-1.5 right-1.5 flex h-3.5 w-3.5 items-center justify-center rounded-full bg-white shadow">
                ...
                <Section title="Header">
                  <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
                    <div className="space-y-1">
                      ...
                      <Input
                        placeholder="Account Enhancement"
                        value={modalConfig.headerTitle}
                        onChange={(e) => setMC('headerTitle', e.target.value)}
                      />
                    </div>
                    <div className="space-y-1">
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          setMC('headerSubtitle', e.target.value)
                        }
                      />
                    </div>
                  </div>
              ...
              <div className="space-y-3 px-4 py-4 sm:px-5">
                <Section title="Content">
                  <div className="space-y-1">
                    ...
                    <Input
                      placeholder="Credits Successfully Allocated"
                      value={modalConfig.headline}
                      onChange={(e) => setMC('headline', e.target.value)}
                    />
                  </div>
                  <div className="space-y-1">
              ...
              <div className="space-y-3 px-4 py-4 sm:px-5">
                <Section title="Highlight Cards">
                  <div className="flex items-center justify-between">
                    ...
                    <Switch
                      checked={modalConfig.showHighlights}
                      onCheckedChange={(v) => setMC('showHighlights', v)}
                    />
                  </div>
                    ...
                    <div className="space-y-2">
                        ...
                        <div key={i} className="flex items-start gap-2">
                          <div className="grid flex-1 grid-cols-1 gap-2 sm:grid-cols-2">
                            <Input
                              ...
                              value={h.title}
                              onChange={(e) =>
                                setHighlight(i, 'title', e.target.value)
                              }
                            />
                            <Input
                              placeholder={
                                i === 0 ? 'Advanced tools' : 'Apply to services'
                              }
                              value={h.description}
                              onChange={(e) =>
                          ...
                          {modalConfig.highlights.length > 1 && (
                            <button
                              ...
                              className="mt-2 text-slate-300 transition-colors hover:text-red-400"
                            >
                      ...
                      {modalConfig.highlights.length < 4 && (
                        <button
                          ...
                          className="text-primary hover:text-primary-hover flex items-center gap-1.5 text-xs font-medium"
                        >
              ...
              <div className="space-y-3 px-4 py-4 sm:px-5">
                <Section title="Recommended Actions">
                  <div className="flex items-center justify-between">
                    ...
                    <Switch
                      checked={modalConfig.showActions}
                      onCheckedChange={(v) => setMC('showActions', v)}
                    />
                  </div>
                    ...
                    <div className="space-y-2">
                      <div className="space-y-1">
                        ...
                        <Input
                          ...
                          value={modalConfig.actionsTitle}
                          onChange={(e) =>
                            setMC('actionsTitle', e.target.value)
                          }
                        />
                      </div>
                ...
                <Section title="Buttons & Footer">
                  <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
                    <div className="space-y-1">
                      ...
                      <Input
                        placeholder="Manage Credits"
                        value={modalConfig.primaryCta}
                        onChange={(e) => setMC('primaryCta', e.target.value)}
                      />
                    </div>
                    <div className="space-y-1">
                      <Label className="text-xs text-slate-500">
                      ...
                      <Input
                        ...
                        value={modalConfig.secondaryCta}
                        onChange={(e) => setMC('secondaryCta', e.target.value)}
                      />
                    </div>
                  </div>
                  <div className="space-y-1">
            ...
            <aside className="min-w-0 bg-slate-50/90 p-4 sm:p-5 lg:sticky lg:top-0 lg:max-h-[min(75vh,880px)] lg:overflow-y-auto">
              ...
              />
              <p className="text-muted-foreground mx-auto mt-3 max-w-[380px] text-center text-[11px]">
                Buttons are inert here; tenants get the real modal after
                distribution.
              </p>
            </aside>
          </div>
