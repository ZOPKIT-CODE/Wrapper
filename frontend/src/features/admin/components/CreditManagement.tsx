...
const CreditManagement: React.FC = () => {
  ...
  const analyzePermissionType = (permCode: string) => {
    ...
    if (code.includes('admin') || code.includes('manage') || code.includes('delete')) {
      return { risk: 'high', icon: <Lock className="w-3 h-3 text-rose-600" /> };
    }
    if (code.includes('write') || code.includes('edit') || code.includes('create') || code.includes('update')) {
      return { risk: 'medium', icon: <Edit className="w-3 h-3 text-amber-600" /> };
    }
    return { risk: 'low', icon: <Eye className="w-3 h-3 text-emerald-600" /> };
  };
  ...
  return (
    <div className="w-full space-y-4 relative pb-10">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-4">
        <TabsList className="grid w-full grid-cols-2 p-1 bg-slate-100 h-10">
          <TabsTrigger value="global" className="flex items-center gap-2 text-[11px] font-black uppercase data-[state=active]:bg-white"><LayoutGrid className="h-3.5 w-3.5" /> Global Standards</TabsTrigger>
          <TabsTrigger value="tenants" className="flex items-center gap-2 text-[11px] font-black uppercase data-[state=active]:bg-white"><Building2 className="h-3.5 w-3.5" /> Tenant Overrides</TabsTrigger>
        </TabsList>

        <TabsContent value="global" className="m-0 space-y-4">
          <CompactMatrix apps={filteredApps} activeTab="global" loading={loading} glass={glass} matrixSearchQuery={matrixSearchQuery} setMatrixSearchQuery={setMatrixSearchQuery} getPermissionStatus={getPermissionStatus} getModuleStats={getModuleStats} getAppStats={getAppStats} handlePermissionChange={handlePermissionChange} handleModuleChange={handleModuleChange} handleAppChange={handleAppChange} getAppIcon={getAppIcon} analyzePermissionType={analyzePermissionType} changeCount={changeCount} onSave={handleSave} onDiscard={() => setCostChanges({})} onPreview={() => setShowComparisonModal(true)} />
        </TabsContent>
        ...
        <TabsContent value="tenants" className="m-0 pt-2">
          <div className="flex gap-6 items-start h-[750px]">
            ...
            <div className="w-[200px] flex-none">
              <Card className="border-slate-200 h-full flex flex-col overflow-hidden shadow-sm rounded-3xl bg-slate-50/50">
                <CardHeader className="p-3 border-b bg-white">
                  <div className="relative">
                    <Search className="absolute left-2.5 top-1/2 -translate-y-1/2 h-3.5 w-3.5 text-slate-400" />
                    <Input placeholder="Filter..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="pl-8 h-8 text-[10px] font-black bg-slate-50 border-none shadow-inner" />
                  </div>
                </CardHeader>
            ...
            <div className="flex-1 min-w-0 h-full">
              {selectedTenant ? (
                <CompactMatrix apps={filteredApps} activeTab="tenants" loading={loading} glass={glass} matrixSearchQuery={matrixSearchQuery} setMatrixSearchQuery={setMatrixSearchQuery} getPermissionStatus={getPermissionStatus} getModuleStats={getModuleStats} getAppStats={getAppStats} handlePermissionChange={handlePermissionChange} handleModuleChange={handleModuleChange} handleAppChange={handleAppChange} getAppIcon={getAppIcon} analyzePermissionType={analyzePermissionType} tenantName={selectedTenant.companyName} changeCount={changeCount} onSave={handleSave} onDiscard={() => setCostChanges({})} />
              ) : (
                <div className="h-full flex items-center justify-center border-2 border-dashed rounded-[40px] bg-slate-50/20 border-slate-200">
                  <div className="text-center">
                    <div className="w-16 h-16 bg-white rounded-3xl flex items-center justify-center mx-auto mb-4 shadow-xl border border-slate-100"><Building2 className="h-8 w-8 text-primary/40" /></div>
                    <h3 className="text-[12px] font-black text-slate-800 uppercase tracking-[0.2em]">Select Infrastructure</h3>
...
const CompactMatrix = ({ apps, activeTab, loading, glass, matrixSearchQuery, setMatrixSearchQuery, getPermissionStatus, getModuleStats, getAppStats, handlePermissionChange, handleModuleChange, handleAppChange, getAppIcon, analyzePermissionType, tenantName, changeCount, onSave, onDiscard }: any) => {
  return (
    <div className={cn("flex flex-col border rounded-3xl overflow-hidden shadow-2xl relative transition-all bg-white", "border-slate-200", glass && "backdrop-blur-xl")}>
      {/* Top Loader */}
      ...
      {/* Header Toolbar */}
      <div className={cn("p-4 border-b flex items-center justify-between gap-4", "bg-slate-50/90")}>
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-[14px] font-black text-primary tracking-tight">{activeTab === 'global' ? 'System Infrastructure' : `${tenantName}`}</h2>
        ...
        <div className="relative flex-1 max-w-sm">
          ...
          <input placeholder="Search matrix..." value={matrixSearchQuery} onChange={(e) => setMatrixSearchQuery(e.target.value)} className={cn("w-full pl-9 pr-3 py-2 rounded-xl text-[11px] font-black outline-none border transition-all", "bg-white border-slate-200 focus:border-primary")} />
        </div>
      ...
      <div className="overflow-y-auto max-h-[700px] custom-scrollbar p-3">
        <Accordion type="single" collapsible className="space-y-2">
          {apps.map((app: any) => {
            ...
            return (
              <AccordionItem key={app.appCode} value={app.appCode} className={cn("border rounded-2xl overflow-hidden transition-all", isAppChanged ? "border-orange-200 bg-orange-50/10" : "bg-slate-50/30")}>
                <AccordionTrigger className="px-5 py-4 hover:no-underline hover:bg-slate-100 transition-all [&[data-state=open]]:bg-primary/5 active:bg-primary/10">
                  <div className="flex items-center justify-between w-full pr-4 text-left">
                    <div className="flex items-center gap-4">
                      <div className="p-2 bg-white rounded-xl shadow-sm border relative">
                        {getAppIcon(app.appCode)}
                        {isAppChanged && <div className="absolute -top-1 -right-1 w-2 h-2 bg-orange-500 rounded-full animate-pulse border-2 border-white" />}
                      </div>
                      <div>
                        <div className="text-[13px] font-black text-primary uppercase flex items-center gap-2 leading-none">
                          {app.appName}
                          <Badge variant="outline" className="text-[8px] h-4 px-1.5 font-bold border-slate-300">{app.appCode}</Badge>
                        </div>
                        <div className="flex items-center gap-2 mt-1">
                          ...
                          {isAppChanged && (
                            <span className="flex items-center gap-1 text-[8px] font-black text-orange-600 uppercase animate-pulse">
                      ...
                      <div className="flex flex-col items-end gap-1">
                        <div className="flex items-center gap-3">
                          ...
                          <div className="relative w-24">
                            <Input type="number" step="0.5" className={cn("h-8 text-[11px] font-black pl-2 pr-6 bg-white focus:ring-1 focus:ring-primary", isAppChanged ? "border-orange-300" : "border-slate-200")} value={appStats.pending || ''} onChange={(e) => handleAppChange(app.appCode, e.target.value)} placeholder="0.0" />
                ...
                <AccordionContent className="p-0 border-t bg-white">
                  <div className="divide-y divide-slate-100">
                    {app.modules?.map((module: any) => {
                      ...
                      return (
                        <div key={module.moduleCode} className={cn("p-4 grid grid-cols-12 gap-6 items-start transition-colors", isModuleChanged ? "bg-orange-50/5" : "hover:bg-slate-50/50")}>
                          {/* Module Column */}
                          <div className="col-span-2 border-r pr-4 space-y-4">
                            <div>
                              <div className="text-[12px] font-black text-blue-600 uppercase tracking-tight">{module.moduleName}</div>
                            ...
                            <div className="space-y-2 pt-2 border-t">
                              <div className="flex justify-between items-center text-[9px] font-black text-slate-500 uppercase">
                                ...
                                <span className="text-slate-900">{stats.avg} CR</span>
                              </div>
                              <div className="relative">
                                <Input type="number" step="0.5" className={cn("h-7 text-[10px] font-black pl-2 pr-7 bg-slate-50/50", isModuleChanged ? "border-orange-300" : "border-slate-200")} value={stats.pending || ''} onChange={(e) => handleModuleChange(app.appCode, module.moduleCode, e.target.value)} placeholder="0.0" />
                                <Package className="absolute right-2 top-1/2 -translate-y-1/2 w-3 h-3 text-blue-400 opacity-30" />
                              </div>
                              {isModuleChanged && (
                                <div className="flex items-center gap-1 text-[8px] font-black text-orange-600 uppercase animate-pulse">
                                  <AlertCircle className="w-2.5 h-2.5" /> Save to Commit
                                </div>
                              )}
                              <Badge className="bg-blue-50 text-blue-600 border-none text-[8px] h-4 w-fit uppercase font-black">Sync Module</Badge>
                            </div>
                          </div>
                              ...
                              {module.permissions?.map((p: any) => {
                                ...
                                return (
                                  <div key={p.code} className={cn(
                                    ...
                                    s.isChanged ? "bg-orange-50/50 border-orange-300 shadow-orange-500/5" :
                                      s.source === 'tenant' ? "bg-blue-50/50 border-blue-300 shadow-blue-500/5" :
                                    ...
                                    <div className="flex-1 min-w-0">
                                      <div className="text-[9.5px] font-black text-slate-900 truncate uppercase leading-tight" title={p.name}>{p.name}</div>
                                    ...
                                    <div className="relative mt-auto pt-2 border-t border-slate-50">
                                      <input
                                        className={cn(
                                          "w-full h-7 px-2 text-[11px] font-black text-right rounded-lg bg-slate-100/30 border transition-all outline-none",
                                      ...
                                      {s.isChanged && <div className="absolute -top-1 -right-1 w-2 h-2 bg-orange-500 rounded-full animate-pulse border-2 border-white" />}
                                    </div>
