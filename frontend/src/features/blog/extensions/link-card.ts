 ...
 * Two variants:
 *   • internal: references another blog post by `postId`. The href is RESOLVED
 *     server-side at render time from the live post (so the card follows slug
 *     changes and drops if the post is unpublished/deleted), like internal Link
 *     references. title/description/image are snapshotted at insert time.
 ...
 * (backend/src/features/blog/extensions/link-card.ts) so the editor and the
...
export const LinkCard = Node.create({
  ...
  addAttributes() {
    return {
      // 'internal' (href resolved from postId at render) | 'external' (stored).
      variant: { default: 'external' },
      // Internal reference target — resolved server-side for the href.
      postId: { default: null },
      // Resolved/stored presentation fields.
      href: { default: null },
      title: { default: null },
