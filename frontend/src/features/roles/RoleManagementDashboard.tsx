...
export function RoleManagementDashboard() {
  ...
  const showRolesLoading =
    rolesLoading || (rolesFetching && rolesData.length === 0)
  ...
  useEffect(() => {
    if (rolesData.length > 0) {
      ...
      const normalizedRoles = rolesData.map((role: DashboardRole) => ({
        ...role,
        permissions: normalizePermissions(role.permissions),
        restrictions:
          typeof role.restrictions === 'string'
            ? (() => {
                try {
      ...
      const filteredRoles = [...normalizedRoles]
    ...
    async (roleId: string) => {
      try {
        ...
        const response = await api.delete(
          `/permissions/roles/${roleId}?force=true`
        )
  ...
  const bulkDeleteRoles = useCallback(
    async (roleIds: string[]) => {
      try {
        const response = await api.post('/permissions/roles/bulk-delete', {
          roleIds,
        })
        if (response.data.success) {
      ...
      >
        <td className="px-5 py-4 align-middle">
          <Checkbox
            ...
            className={cn(
              actualTheme === 'dark'
                ? 'border-purple-500/30 data-[state=checked]:bg-purple-600'
                : actualTheme === 'monochrome'
                  ? 'border-gray-500/30 data-[state=checked]:bg-gray-600'
                  : ''
            )}
          />
        </td>
        ...
        <td className="px-5 py-4 align-middle">
          <div className="flex flex-wrap items-center justify-center gap-2">
            {permissionSummary.admin > 0 && (
              <div
                className="flex items-center gap-1.5 rounded-full border border-rose-100 bg-rose-50 px-2 py-0.5 dark:border-rose-500/20 dark:bg-rose-500/10"
            ...
            {permissionSummary.write > 0 && (
              <div
                className="flex items-center gap-1.5 rounded-full border border-amber-100 bg-amber-50 px-2 py-0.5 dark:border-amber-500/20 dark:bg-amber-500/10"
            ...
            {permissionSummary.read > 0 && (
              <div
                className="flex items-center gap-1.5 rounded-full border border-emerald-100 bg-emerald-50 px-2 py-0.5 dark:border-emerald-500/20 dark:bg-emerald-500/10"
          ...
          <div className="flex flex-col items-center gap-1">
            <Badge
              ...
              className={cn(
                ...
                actualTheme === 'dark'
                  ? role.isSystemRole
                    ? 'border-blue-500/40 bg-[#1B2E5A]/30 text-blue-300'
                    : 'border-slate-600 bg-slate-700 text-slate-300'
                  : 'border-slate-200 bg-slate-100 text-slate-700 dark:border-slate-600 dark:bg-slate-700 dark:text-slate-300'
              )}
              style={{
                fontFamily: 'var(--zk-mono)',
                fontSize: 10,
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button
                variant="ghost"
                size="sm"
                className="h-9 w-9 rounded-lg p-0 hover:bg-slate-100 hover:ring-1 hover:ring-slate-200 dark:hover:bg-slate-800 dark:hover:ring-slate-600"
              ...
              {!isReadOnlyRole && (
                <>
                  ...
                  <DropdownMenuItem
                    ...
                    className="gap-2 text-rose-500 focus:bg-rose-500/10 focus:text-rose-400"
                  >
            ...
            <div className="flex-1 space-y-2">
              ...
              <div className="relative">
                ...
                <Input
                  ...
                  className={`pl-11 ${
                    actualTheme === 'dark'
                      ? 'border-purple-500/30 bg-slate-800/50 text-white placeholder-purple-300 focus:ring-purple-500'
                      : actualTheme === 'monochrome'
                        ? 'border-gray-500/30 bg-gray-800/50 text-gray-100 placeholder-gray-400 focus:ring-gray-400'
                        : ''
                  }`}
                />
              </div>
            ...
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={(value: string) =>
                  setTypeFilter(value as 'all' | 'custom' | 'system')
                }
              >
                <SelectTrigger
                  className={
                    actualTheme === 'dark'
                      ? 'border-purple-500/30 bg-slate-800/50 text-white'
                      : actualTheme === 'monochrome'
                        ? 'border-gray-500/30 bg-gray-800/50 text-gray-100'
                        : ''
                  }
                >
                  <SelectValue />
            ...
            <div className="flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
              ...
              <div className="flex flex-wrap gap-2">
                <Button
                  ...
                  size="sm"
                  onClick={() =>
                    handleBulkAction('export', Array.from(selectedRoles))
                  }
                  className={
                    actualTheme === 'dark'
                ...
                <Button
                  ...
                  size="sm"
                  onClick={() =>
                    handleBulkAction('deactivate', Array.from(selectedRoles))
                  }
                  className={
                    actualTheme === 'dark'
                      ? 'border-purple-500/30 text-purple-300 hover:bg-purple-500/20'
                ...
                <Button
                  ...
                  size="sm"
                  onClick={() =>
                    handleBulkAction('delete', Array.from(selectedRoles))
                  }
                  className={
                    actualTheme === 'dark'
      ...
      >
        {showRolesLoading ? (
          <CardContent className="p-12 text-center">
            <ZopkitRoundLoader
              size="xl"
              className={cn('mx-auto', actualTheme === 'dark' && 'text-white')}
            />
            <p
              className={cn(
          ...
          <table className="w-full min-w-[1000px] border-collapse">
            <thead
              className={cn(
                'sticky top-0 z-20',
                actualTheme === 'dark'
                  ? 'border-b border-slate-700 bg-slate-800/95'
                  : actualTheme === 'monochrome'
            ...
            >
              ...
              >
                <th className="w-12 px-5 py-4 text-left">
                  <Checkbox
                    checked={
                      selectedRoles.size === roles.length && roles.length > 0
                    }
                    onCheckedChange={
                ...
                <th className="min-w-[220px] px-5 py-4 text-left">
                  Role & Description
                </th>
                <th className="w-24 px-5 py-4 text-center">Users</th>
                <th className="w-32 px-5 py-4 text-center">Apps/Modules</th>
                <th className="px-5 py-4 text-center">
                  <div className="flex flex-col items-center gap-1">
                    <span>Capability Breakdown</span>
                    <div className="flex items-center gap-3 text-[9px] font-normal tracking-wide uppercase opacity-70">
                ...
                <th className="w-32 px-5 py-4 text-center">Type</th>
        ...
        <DialogContent
          className={
            actualTheme === 'dark'
              ...
              : actualTheme === 'monochrome'
                ? 'border-gray-500/30 bg-gray-900 text-gray-100'
                : ''
          }
        >
          <DialogHeader>
