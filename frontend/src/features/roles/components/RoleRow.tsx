...
const normalizePermissions = (
  permissions: Record<string, unknown> | string[] | string
...
// API may attach pre-computed roll-up counts that aren't part of DashboardRole.
type RoleWithComputedCounts = DashboardRole & {
...
export function RoleRow({
  ...
  const roleWithCounts = role as RoleWithComputedCounts
  const displayCount = roleWithCounts.permissionCount || permissionSummary.total
  const displayModules = roleWithCounts.moduleCount || permissionSummary.modules
  const displayApps =
        ...
        <div className="space-y-2">
          ...
          <div className="flex items-center gap-4 text-sm">
            ...
            <div className="flex items-center gap-2">
              ...
              <span style={{ color: 'var(--zk-muted)', fontSize: 13 }}>
                modules
              </span>
              <span
                style={{
          ...
          <div className="flex items-center gap-3">
            {permissionSummary.admin > 0 && (
              <div className="flex items-center gap-1">
                <div
                  className={`h-2 w-2 rounded-full ${getPermissionTypeColor('admin')}`}
                ></div>
                <span
                  className={`text-xs font-medium ${getPermissionTypeTextColor('admin')}`}
                >
            ...
            {permissionSummary.write > 0 && (
              <div className="flex items-center gap-1">
                <div
                  className={`h-2 w-2 rounded-full ${getPermissionTypeColor('write')}`}
                ></div>
                <span
                  className={`text-xs font-medium ${getPermissionTypeTextColor('write')}`}
                >
            ...
            {permissionSummary.read > 0 && (
              <div className="flex items-center gap-1">
                <div
                  className={`h-2 w-2 rounded-full ${getPermissionTypeColor('read')}`}
                ></div>
                <span
                  className={`text-xs font-medium ${getPermissionTypeTextColor('read')}`}
                >
