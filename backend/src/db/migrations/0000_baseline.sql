...
CREATE TABLE public.applications (
    ...
    CONSTRAINT chk_application_status CHECK (((status)::text = ANY ((ARRAY['active'::character varying, 'inactive'::character varying, 'maintenance'::character varying, 'deprecated'::character varying])::text[])))
);
...
CREATE TABLE public.credit_batches (
    ...
    CONSTRAINT credit_batches_credit_type_check CHECK (((credit_type)::text = ANY ((ARRAY['free'::character varying, 'paid'::character varying, 'seasonal'::character varying])::text[])))
);
...
CREATE TABLE public.credit_purchases (
    ...
    created_at timestamp with time zone DEFAULT now(),
    CONSTRAINT chk_credit_purchase_status CHECK (((status)::text = ANY ((ARRAY['pending'::character varying, 'completed'::character varying, 'failed'::character varying, 'cancelled'::character varying, 'processing'::character varying])::text[])))
);
...
CREATE TABLE public.credit_transactions (
    ...
    initiated_by uuid,
    created_at timestamp with time zone DEFAULT now(),
    CONSTRAINT chk_credit_transaction_type CHECK (((transaction_type)::text = ANY ((ARRAY['purchase'::character varying, 'consumption'::character varying, 'expiry'::character varying, 'adjustment'::character varying, 'transfer'::character varying, 'initialization'::character varying, 'allocation'::character varying, 'transfer_in'::character varying, 'transfer_out'::character varying])::text[])))
);
...
CREATE TABLE public.entities (
    ...
    CONSTRAINT chk_entity_type CHECK (((entity_type)::text = ANY ((ARRAY['organization'::character varying, 'location'::character varying, 'department'::character varying, 'team'::character varying])::text[])))
);
...
CREATE TABLE public.organization_memberships (
    ...
    created_at timestamp with time zone DEFAULT now(),
    updated_at timestamp with time zone DEFAULT now(),
    CONSTRAINT chk_membership_status CHECK (((membership_status)::text = ANY ((ARRAY['active'::character varying, 'inactive'::character varying, 'suspended'::character varying, 'pending'::character varying, 'invited'::character varying])::text[]))),
    CONSTRAINT chk_org_membership_entity_type CHECK (((entity_type)::text = ANY ((ARRAY['organization'::character varying, 'location'::character varying, 'department'::character varying, 'team'::character varying])::text[])))
);
...
CREATE TABLE public.payments (
    ...
    CONSTRAINT chk_payment_status CHECK (((status)::text = ANY ((ARRAY['pending'::character varying, 'completed'::character varying, 'failed'::character varying, 'refunded'::character varying, 'processing'::character varying, 'cancelled'::character varying])::text[])))
);
...
CREATE TABLE public.subscriptions (
    ...
    CONSTRAINT chk_subscription_status CHECK (((status)::text = ANY ((ARRAY['active'::character varying, 'inactive'::character varying, 'trialing'::character varying, 'past_due'::character varying, 'canceled'::character varying, 'paused'::character varying, 'trial'::character varying])::text[])))
);
...
CREATE TABLE public.tenant_invitations (
    ...
    CONSTRAINT chk_invitation_scope CHECK (((invitation_scope)::text = ANY ((ARRAY['tenant'::character varying, 'single-entity'::character varying, 'multi-entity'::character varying])::text[]))),
    CONSTRAINT chk_invitation_status CHECK (((status)::text = ANY ((ARRAY['pending'::character varying, 'accepted'::character varying, 'expired'::character varying, 'cancelled'::character varying, 'revoked'::character varying])::text[])))
);
...
--
-- Name: audit_logs audit_logs_user_id_tenant_users_user_id_fk; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.audit_logs
    ADD CONSTRAINT audit_logs_user_id_tenant_users_user_id_fk FOREIGN KEY (user_id) REFERENCES public.tenant_users(user_id);
