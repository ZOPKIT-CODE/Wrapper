...
export class SeasonalCreditService {
  ...
  static async createDistributionCampaign(campaignData: Record<string, unknown>): Promise<typeof seasonalCreditCampaigns.$inferSelect> {
    ...
    Logger.log('info', 'billing', 'create-campaign', 'Created seasonal credit campaign', { campaignId: campaign.campaignId, allocationMode, targetApplications: normalizedTargetApplications });
    return campaign;
  }
  ...
  static async addCreditsToOrganization(tenantId: string, entityId: string, creditAmount: number | string, campaignId: string, _campaignName: string): Promise<{ previousBalance: number; newBalance: number; creditAmount: number | string }> {
    ...
    Logger.log('info', 'billing', 'add-credits', 'Added credits to tenant entity', { creditAmount, tenantId, entityId });
  ...
  static async createCreditDistributionNotification(campaign: Record<string, unknown>, tenantId: string, creditAmount: number | string): Promise<void> {
    ...
    Logger.log('info', 'billing', 'create-notification', 'Created notification for tenant', { tenantId });
  }
  ...
  static async distributeCreditsToTenants(campaignId: string): Promise<Record<string, unknown>> {
    ...
    Logger.log('info', 'billing', 'distribute-credits', 'Starting credit distribution for campaign', { campaignId, campaignName: campaign.campaignName });
    ...
    Logger.log('info', 'billing', 'distribute-credits', 'Distributing credits to tenants', { tenantCount: targetTenantIds.length });
    ...
    for (const tenantId of targetTenantIds) {
      try {
        ...
        if (!primaryEntity) {
          Logger.log('warning', 'billing', 'distribute-credits', 'No primary organization found for tenant', { tenantId });
          failedCount++;
        ...
        if (allocationMode === 'primary_org') {
          ...
          // Create separate allocation record for each target application (batch insert)
          if (targetApplications.length > 0) {
            await db.insert(creditBatches)
              .values(targetApplications.map(targetApp => ({
                campaignId: String(campaignId),
                tenantId,
                entityId: primaryEntity.entityId,
                entityType: 'organization',
          ...
          Logger.log('info', 'billing', 'distribute-credits', 'Created application-specific allocations for tenant', { count: targetApplications.length, tenantId });
        }
        ...
        distributedCount++;
        Logger.log('info', 'billing', 'distribute-credits', 'Distributed credits to tenant', { creditsToAllocate, tenantId });

      } catch (err: unknown) {
        const error = err as Error;
        Logger.log('error', 'billing', 'distribute-credits', 'Failed to distribute credits to tenant', { tenantId, error: error.message });
    ...
    Logger.log('info', 'billing', 'distribute-credits', 'Distribution complete', { distributedCount, failedCount });
  ...
  static async extendCampaignExpiry(campaignId: string, additionalDays: number): Promise<Record<string, unknown>> {
    ...
    Logger.log('info', 'billing', 'extend-campaign', 'Extended campaign expiry', { campaignId, additionalDays });
  ...
  static async sendExpiryWarnings(daysAhead = 7): Promise<Record<string, unknown>> {
    ...
    if (expiringAllocations.length > 0) {
      try {
        await db.insert(notifications)
          .values(expiringAllocations.map(allocation => ({
        ...
        emailsSent = expiringAllocations.length;
      } catch (err: unknown) {
        const error = err as Error;
        Logger.log('error', 'billing', 'send-expiry-warnings', 'Failed to batch-insert expiry warning notifications', { error: error.message });
      }
    }

    Logger.log('info', 'billing', 'send-expiry-warnings', 'Sent expiry warnings', { emailsSent });
  ...
  static async processExpiries(): Promise<Record<string, unknown>> {
    ...
    for (const allocation of expiredAllocations) {
      try {
        ...
        processedCount++;
        Logger.log('info', 'billing', 'process-expiries', 'Processed expiry for allocation', { allocationId: allocation.allocationId });

      } catch (err: unknown) {
        const error = err as Error;
        Logger.log('error', 'billing', 'process-expiries', 'Failed to process expiry for allocation', { allocationId: allocation.allocationId, error: error.message });
      }
    }

    Logger.log('info', 'billing', 'process-expiries', 'Processed expired allocations', { processedCount });
  ...
  static async bulkExpireBatches(
    ...
    for (const batch of batches) {
      try {
        ...
        errorCount++;
        Logger.log('error', 'billing', 'bulk-expire-batches', 'Error in bulkExpireBatches', { error: (err as Error).message });
      }
    }
  ...
  static async grantSeasonalCreditsToTenant(params: {
    ...
    const suiteAppsGrant = (process.env.BUSINESS_SUITE_TARGET_APPS || 'crm,accounting,ops').split(',').map((a: string) => a.trim());
    for (const targetApp of suiteAppsGrant) {
      snsSqsPublisher.publishCreditEvent(targetApp, 'credit.granted', tenantId, {
        entityId,
      ...
      }, initiatedBy).catch((err: Error) => {
        Logger.log('warning', 'billing', 'grant-seasonal-credits', 'Failed to publish credit.granted event', { tenantId, error: err.message });
      });
    }
