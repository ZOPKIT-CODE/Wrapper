...
export class TenantAdminService {
  ...
  static async getTenantList(filters: Record<string, unknown> = {}, pagination: Record<string, unknown> = {}): Promise<Record<string, unknown>> {
    try {
      ...
      Logger.log('error', 'general', 'get-tenant-list', 'Error getting tenant list', { error: error.message });
      throw error;
    }
  }
  ...
  static async getTenantCount(filters: Record<string, unknown> = {}): Promise<number> {
    try {
      ...
      const error = err as Error;
      Logger.log('error', 'general', 'get-tenant-count', 'Error getting tenant count', { error: error.message });
      throw error;
    }
  }
  ...
  static async getTenantDetails(tenantId: string): Promise<Record<string, unknown>> {
    try {
      ...
      const error = err as Error;
      Logger.log('error', 'general', 'get-tenant-details', 'Error getting tenant details', { error: error.message });
      throw error;
    }
  }
  ...
  static async updateTenantStatus(tenantId: string, isActive: boolean, reason: string | null = null, adminUserId: string | null = null): Promise<Record<string, unknown>> {
    try {
      ...
      Logger.log('info', 'general', 'update-tenant-status', 'Tenant status updated by admin', { tenantId, isActive, adminUserId, reason });

      return { success: true };
    } catch (err: unknown) {
      const error = err as Error;
      Logger.log('error', 'general', 'update-tenant-status', 'Error updating tenant status', { error: error.message });
      throw error;
    }
  }
  ...
  static async bulkUpdateTenantStatus(tenantIds: string[], isActive: boolean, reason: string | null = null, adminUserId: string | null = null): Promise<Record<string, unknown>> {
    try {
      ...
      Logger.log('info', 'general', 'bulk-update-tenant-status', 'Bulk tenant status updated by admin', { count: tenantIds.length, isActive, adminUserId, reason });
      ...
      const error = err as Error;
      Logger.log('error', 'general', 'bulk-update-tenant-status', 'Error bulk updating tenant status', { error: error.message });
      throw error;
    }
  }
  ...
  static async getTenantActivity(tenantId: string, limit = 50, offset = 0): Promise<unknown[]> {
    try {
      ...
      const error = err as Error;
      Logger.log('error', 'general', 'get-tenant-activity', 'Error getting tenant activity', { error: error.message });
      throw error;
    }
  }
  ...
  static async exportTenantData(tenantId: string): Promise<Record<string, unknown>> {
    try {
      ...
      const error = err as Error;
      Logger.log('error', 'general', 'export-tenant-data', 'Error exporting tenant data', { error: error.message });
      throw error;
    }
  }
  ...
  static async getTenantStats(tenantId: string): Promise<Record<string, unknown>> {
    try {
      ...
      const error = err as Error;
      Logger.log('error', 'general', 'get-tenant-stats', 'Error getting tenant stats', { error: error.message });
      throw error;
    }
  }
